...
<HTML lang=zh-cn><HEAD><TITLE>JavaScript If...Else 语句</TITLE>
<META charset=gbk>
...
<UL>
  ...
  <LI class=currentLink><A title="JavaScript If...Else 语句" 
  href="js_if_else.asp">JS If...Else</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="JavaScript 比较和逻辑运算符" href="js_comparisons.asp">JS 
  比较</A> </LI>
  <LI class=next><A title="JavaScript Switch 语句" href="js_switch.asp">JS 
  Switch</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>条件语句用于基于不同的条件来执行不同的动作。</STRONG></P></DIV>
<DIV>
<H2>条件语句</H2>
<P>通常在写代码时，您总是需要为不同的决定来执行不同的动作。您可以在代码中使用条件语句来完成该任务。</P>
<P>在 JavaScript 中，我们可使用以下条件语句：</P>
<UL>
  <LI><EM>if 语句</EM> - 只有当指定条件为 true 时，使用该语句来执行代码 
  <LI><EM>if...else 语句</EM> - 当条件为 true 时执行代码，当条件为 false 时执行其他代码 
  <LI><EM>if...else if....else 语句</EM> - 使用该语句来选择多个代码块之一来执行 
  <LI><EM>switch 语句</EM> - 使用该语句来选择多个代码块之一来执行 </LI></UL></DIV>
<DIV>
<H2>If 语句</H2>
...
<H3>实例</H3>
<P>当时间小于 20:00 时，生成一个“Good day”问候：</P><PRE>if (time&lt;20)
  {
  x="Good day";
  }
</PRE>
<P>x 的结果是：</P><PRE><SCRIPT>
var d=new Date();
var time=d.getHours();
if (time<20)
  {
...
<P>请注意，在这个语法中，没有 ..else..。您已经告诉浏览器只有在指定条件为 true 时才执行代码。</P></DIV>
<DIV>
<H2>If...else 语句</H2>
...
<H3>实例</H3>
<P>当时间小于 20:00 时，将得到问候 "Good day"，否则将得到问候 "Good evening"。</P><PRE>if (time&lt;20)
  {
  x="Good day";
  }
else
...
<P class=tiy><A href="../tiy/t.asp@f=js_ifthenelse">亲自试一试</A></P></DIV>
<DIV>
<H2>If...else if...else 语句</H2>
<P>使用 if....else if...else 语句来选择多个代码块之一来执行。</P>
<H3>语法</H3><PRE>if (条件 1)
  {
  当条件 1 为 true 时执行的代码
  }
else if (条件 2)
...
<H3>实例</H3>
<P>如果时间小于 10:00，则将发送问候 "Good morning"，否则如果时间小于 20:00，则发送问候 "Good day"，否则发送问候 
"Good evening"：</P><PRE>if (time&lt;10)
  {
  x="Good morning";
  }
else if (time&lt;20)
...
<P class=tiy><A href="../tiy/t.asp@f=js_elseif">亲自试一试</A></P></DIV>
<DIV class=example>
<H2>更多实例</H2>
<DL>
  <DT>随机的链接 
  <DD>本例将输出 W3School 或微软公司的链接。通过使用随机数，每个链接被输出的机会为 50%。 </DD></DL></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="JavaScript 比较和逻辑运算符" href="js_comparisons.asp">JS 
  比较</A> </LI>
...
<DL>
  <DT><A title="ECMAScript if 语句" href="pro_js_statements_if.asp">ECMAScript if 
  语句</A> 
  <DD>if 语句是 ECMAScript 中最常用的语句之一。本节为您详细讲解了如何使用 if 语句。 </DD></DL></DIV></DIV><!-- maincontent end -->
<DIV id=sidebar>
<DIV id=searchui>
