...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP.NET - DataList 控件</TITLE>
...
<UL>
  ...
  Repeater</A> 
  <LI class=currentLink><A title="WebForms DataList" 
  href="aspnet_datalist.asp">WebForms DataList</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms Repeater" href="aspnet_repeater.asp">WebForms 
  Repeater</A> </LI>
  <LI class=next><A title="WebForms DbConnection" 
  href="aspnet_dbconnection.asp">WebForms 数据库连接</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>DataList 控件，类似于 Repeater 控件，用于显示限制于该控件的项目的重复列表。不过，DataList 
控件会默认地在数据项目上添加表格。 </STRONG></P></DIV>
<DIV class=example>
<H2>实例</H2>
<DL>
  <DT><A href="../tiy/s.asp@f=demo_aspnet_datalist_1">DataList 控件</A> 
  <DT><A href="../tiy/s.asp@f=demo_aspnet_datalist_2">带有样式的 DataList 控件</A> 
  <DT><A href="../tiy/s.asp@f=demo_aspnet_datalist_3">带有 
  &lt;AlternatingItemTemplate&gt; 的 DataList 控件</A> </DT></DL></DIV>
<DIV>
<H2>把 DataSet 绑定到 DataList 控件</H2>
<P>DataList 控件，类似于 Repeater 控件，用于显示限制于该控件的项目的重复列表。不过，DataList 
控件会默认地在数据项目上添加表格。DataList 控件可被绑定到数据库表、XML 文件或者其他项目列表。这里，我们将展示如何把 XML 文件绑定到一个 
DataList 控件。</P>
...
<P>然后我们在 .aspx 页面中创建一个 DataList 控件。&lt;HeaderTemplate&gt; 元素中的内容在输出中仅出现一次，而 
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P>然后我们添加可创建 DataSet 的脚本，并把这个 mycdcatalog DataSet 绑定到 DataList 控件。我们同样使用这些元素来填充该 
DataList 控件：包含表头的 &lt;HeaderTemplate&gt;，包含要显示的数据项的 &lt;ItemTemplate&gt;，以及包含文本的 
&lt;FooterTemplate&gt;。请注意，DataList 的 gridlines 属性被设置为 "both"，以显示表格边框：</P><PRE>&lt;%@ Import Namespace="System.Data" %&gt;
...
&lt;/html&gt;</PRE>
<P class=tiy><A 
href="../tiy/s.asp@f=demo_aspnet_datalist_1">显示这个例子</A></P></DIV>
<DIV>
<H2>使用样式</H2>
<P>您也可以向 DataList 控件添加样式，让它更加时髦：</P><PRE>&lt;%@ Import Namespace="System.Data" %&gt;
...
&lt;/html&gt;</PRE>
<P class=tiy><A 
href="../tiy/s.asp@f=demo_aspnet_datalist_2">显示这个例子</A></P></DIV>
<DIV>
<H2>使用 &lt;AlternatingItemTemplate&gt;</H2>
<P>您可以在 &lt;ItemTemplate&gt; 元素后添加 &lt;AlternatingItemTemplate&gt; 
元素，这样就可以描述交替行的外观了。您可以在 DataList 控件内部对 &lt;AlternatingItemTemplate&gt; 
部分的数据进行样式化处理：</P><PRE>&lt;%@ Import Namespace="System.Data" %&gt;
...
&lt;/html&gt;</PRE>
<P class=tiy><A 
href="../tiy/s.asp@f=demo_aspnet_datalist_3">显示这个例子</A></P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms Repeater" href="aspnet_repeater.asp">WebForms 
  Repeater</A> </LI>
  <LI class=next><A title="WebForms DbConnection" 
  href="aspnet_dbconnection.asp">WebForms 数据库连接</A> </LI></UL></DIV></DIV>
<DIV id=sidebar>
<DIV id=searchui>
