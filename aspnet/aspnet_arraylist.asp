...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP NET - ArrayList 对象</TITLE>
...
<UL>
  ...
  数据绑定</A> 
  <LI class=currentLink><A title="WebForms ArrayList" 
  href="aspnet_arraylist.asp">WebForms ArrayList</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms Data Binding" 
  href="aspnet_databinding.asp">WebForms 数据绑定</A> </LI>
  <LI class=next><A title="WebForms Hashtable" 
  href="aspnet_hashtable.asp">WebForms Hashtable</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>ArrayList 对象是包含单一数据值的项目的集合。</STRONG></P></DIV>
<DIV>
<H2>实例</H2>
<P class=tiy><A href="../tiy/s.asp@f=demo_aspnet_arraylist_drop_1">ArrayList 
DropDownList</A></P>
<P class=tiy><A href="../tiy/s.asp@f=demo_aspnet_arraylist_radio_1">ArrayList 
RadioButtonList</A></P></DIV>
<DIV>
<H2>创建 ArrayList</H2>
<P>ArrayList 对象是包含单一数据值的项目的集合。</P>
<P>通过 Add() 方法向 ArrayList 添加项目。</P>
<P>下面的代码创建了一个新的 ArrayList 对象，名为 mycountries，并添加了四个项目：</P><PRE>&lt;script runat="server"&gt;
Sub Page_Load
...
&lt;/script&gt;</PRE>
<P>默认地，一个 ArrayList 对象包含 16 个条目。可通过 TrimToSize() 方法把 ArrayList 调整为最终大小：</P><PRE>&lt;script runat="server"&gt;
Sub Page_Load
if Not Page.IsPostBack then
...
&lt;/script&gt;</PRE>
<P>通过 Sort() 方法，ArrayList 也能够按照字母顺序或者数字顺序进行排序：</P><PRE>&lt;script runat="server"&gt;
Sub Page_Load
if Not Page.IsPostBack then
  dim mycountries=New ArrayList
  mycountries.Add("China")
...
&lt;/script&gt;</PRE>
<P>要实现颠倒的排序，请在 Sort() 方法后应用 Reverse() 方法：</P><PRE>&lt;script runat="server"&gt;
Sub Page_Load
if Not Page.IsPostBack then
  dim mycountries=New ArrayList
  mycountries.Add("China")
...
<UL>
  <LI>asp:RadioButtonList 
  <LI>asp:CheckBoxList 
  <LI>asp:DropDownList 
  <LI>asp:Listbox </LI></UL>
<P>如需把数据绑定到一个 RadioButtonList 控件，首先请在一个 .aspx 页面中创建 RadioButtonList 控件（请注意，没有任何 
asp:ListItem 元素）：</P><PRE>&lt;html&gt;
&lt;body&gt;
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P>然后添加构建列表的脚本，并把列表中的值绑定到该 RadioButtonList 控件：</P><PRE>&lt;script runat="server"&gt;
Sub Page_Load
if Not Page.IsPostBack then
  dim mycountries=New ArrayList
  mycountries.Add("China")
...
&lt;/html&gt;</PRE>
<P class=tiy><A 
href="../tiy/s.asp@f=demo_aspnet_arraylist_radio_1">显示这个例子</A></P>
<P>RadioButtonList 控件的 DataSource 属性被设置为该 ArrayList，它定义了这个 RadioButtonList 
控件的数据源。RadioButtonList 控件的 DataBind() 方法把 RadioButtonList 控件与数据源绑定在一起。</P>
<P class=note><SPAN>注释：</SPAN>数据值作为控件的 Text 和 Value 属性来使用。如需添加不同于 Text 的 
Value，既可以使用 Hashtable 对象，也可以使用 SortedList 对象。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms Data Binding" 
  href="aspnet_databinding.asp">WebForms 数据绑定</A> </LI>
  <LI class=next><A title="WebForms Hashtable" 
  href="aspnet_hashtable.asp">WebForms Hashtable</A> </LI></UL></DIV></DIV>
<DIV id=sidebar>
<DIV id=searchui>
