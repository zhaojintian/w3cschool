...
<HTML lang=zh-cn><HEAD><TITLE>ASP.NET Web Pages - 发布网站</TITLE>
<META charset=gb2312>
...
<UL>
  ...
  <LI class=currentLink><A title="WebPages 发布" 
  href="webpages_publish.asp">WebPages 发布</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebPages PHP" href="webpages_php.asp">WebPages PHP</A> 
  </LI>
  <LI class=next><A title="WebPages 实例" href="webpages_examples.asp">WebPages 
  实例</A> </LI></UL></DIV>
<DIV id=intro>
<P>学习如何在不使用 WebMatrix 的情况下发布 Web Pages 应用程序。</P></DIV>
<DIV>
<H2>在不使用 WebMatrix 的情况下发布您的应用程序</H2>
<P>可以在 WebMatrix （、Visual Web Developer 或 Visual Studio) 中使用 Publish 命令把 ASP.NET 
Web Pages 应用程序发布到远程服务器。</P>
<P>该功能会复制所有应用程序文件、cshtml 页面、图片以及所有必需的 DLL 文件，这些文件用于 Web Pages、Razor、Helpers 以及 
SQL Server Compact（如果使用了数据库）。</P>
...
<P>在您继续之前，请确保您的主机运行最新的 ASP.NET 版本（4.0 或 4.5）。</P></DIV>
<DIV>
<H2>2. 复制 Web 文件夹</H2>
<P>从开发机上把网站（所有文件夹和内容）复制到远程主机（服务器）上的应用程序文件夹。</P>
<P>如果您的应用程序包含数据，<B>不要复制这些数据</B>（参见下面的步骤 4）。</P></DIV>
<DIV>
<H2>3. DLL 文件</H2>
<P>确保远程主机上的 bin 文件夹包含与开发机上相同的 dll 文件。</P>
<P>在复制 bin 文件夹之后，它应该包含这些文件：</P>
<UL>
  <LI>Microsoft.Web.Infrastructure.dll 
  <LI>NuGet.Core.dll 
  <LI>System.Web.Helpers.dll 
  <LI>System.Web.Razor.dll 
  <LI>System.Web.WebPages.Administration.dll 
  <LI>System.Web.WebPages.Deployment.dll 
  <LI>System.Web.WebPages.dll 
  <LI>System.Web.WebPages.Razor.dll 
  <LI>WebMatrix.Data.dll 
  <LI>WebMatrix.WebData </LI></UL></DIV>
<DIV>
<H2>4. 复制数据</H2>
<P>如果您的应用程序包含数据或数据库。例如 SQL Server Compact 数据库（App_Data 文件夹中的 .sdf 
文件），请考虑以下内容：</P>
...
<P><B>就是这些。祝你好运！</B></P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebPages PHP" href="webpages_php.asp">WebPages PHP</A> 
  </LI>
  <LI class=next><A title="WebPages 实例" href="webpages_examples.asp">WebPages 
  实例</A> </LI></UL></DIV></DIV><!-- maincontent end -->
<DIV id=sidebar>
<DIV id=searchui>
