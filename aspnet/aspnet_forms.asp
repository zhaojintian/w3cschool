...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP.NET Web 窗体</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="WebForms 窗体" href="aspnet_forms.asp">WebForms 
  窗体</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms 事件" href="aspnet_events.asp">WebForms 事件</A> 
  </LI>
  <LI class=next><A title="WebForms ViewState" 
  href="aspnet_viewstate.asp">WebForms ViewState</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>所有服务器控件必须出现在 &lt;form&gt; 标签内，同时 &lt;form&gt; 标签必须包含 runat="server" 
属性。</STRONG></P></DIV>
<DIV>
<H2>ASP.NET Web 表单</H2>
<P>所有服务器控件必须出现在 &lt;form&gt; 标签内，同时 &lt;form&gt; 标签必须包含 runat="server" 
属性。runat="server" 属性指示该表单必须在服务器上进行处理。它还指示装入其中的控件能否被服务器脚本访问：</P><PRE>&lt;form runat="server"&gt;...HTML + server controls&lt;/form&gt;</PRE>
<P class=note><SPAN>注释：</SPAN>该表单总是向自身页面进行提交。如果您规定了一个 action 属性，它会被忽略。如果您省略了 
method 属性，它将被默认地设置为 method="post"。同时，如果您没有规定 name 和 id 属性，它们则由 ASP.NET 自动分配。</P>
<P class=note><SPAN>注释：</SPAN>一个 .aspx 仅能包含一个 &lt;form runat="server"&gt; 
控件！</P>
<P>如果您查看一个 .aspx 页面的源代码，而其中包含的表单不带有 name, method, action 或 id 属性，那么您将看到 ASP.NET 
已经把这些属性添加到该表单。类似这样：</P><PRE>&lt;form name="_ctl0" method="post" action="page.aspx" id="_ctl0"&gt;...some code&lt;/form&gt;</PRE></DIV>
<DIV>
<H2>提交表单</H2>
<P>表单通常通过点击一个按钮来提交。ASP.NET 中的 Button 服务器控件的格式是这样的：</P><PRE>&lt;asp:Button id="<I>id</I>" text="<I>label</I>" OnClick="<I>sub</I>" runat="server" /&gt;</PRE>
...
<P class=tiy><A href="../tiy/s.asp@f=demo_aspnet_button">显示这个例子</A></P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms 事件" href="aspnet_events.asp">WebForms 事件</A> 
  </LI>
  <LI class=next><A title="WebForms ViewState" 
  href="aspnet_viewstate.asp">WebForms ViewState</A> </LI></UL></DIV></DIV>
<DIV id=sidebar>
<DIV id=searchui>
