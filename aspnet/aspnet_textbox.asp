...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP .NET - TextBox 控件</TITLE>
...
<UL>
  ...
  ViewState</A> 
  <LI class=currentLink><A title="WebForms TextBox" 
  href="aspnet_textbox.asp">WebForms TextBox</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms ViewState" 
  href="aspnet_viewstate.asp">WebForms ViewState</A> </LI>
  <LI class=next><A title="WebForms Button" href="aspnet_button.asp">WebForms 
  Button</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>TextBox 控件用于创建用户可输入文本的文本框。</STRONG></P></DIV>
<DIV>
<H2>TextBox 控件</H2>
<P>TextBox 控件用于创建用户可输入文本的文本框。</P>
<P>TextBox 控件的属性列在我们的 <A title="ASP.NET TextBox 控件" 
href="control_textbox.asp">TextBox 控件参考手册</A>中。</P>
<P>下面的例子演示了您可能在 TextBox 控件中使用到的一些属性：</P><PRE>&lt;html&gt;
&lt;body&gt;
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P class=tiy><A href="../tiy/s.asp@f=demo_aspnet_textbox_1">显示这个例子</A></P></DIV>
<DIV>
<H2>添加脚本</H2>
<P>当表单被提交时，TextBox 控件的内容和设置可通过服务器脚本进行修改。可通过点击一个按钮或当用户更改 TextBox 
控件中的值对表单进行提交。</P>
<P>在下面的例子中，我们在一个 .aspx 文件中声明了一个 TextBox 控件、一个 Button 控件和一个 Label 
控件。当提交按钮被触发时，submit 子例程就会被执行。submit 子例程会向 Label 控件写一条文本：</P><PRE>&lt;script runat="server"&gt;
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P class=tiy><A href="../tiy/s.asp@f=demo_aspnet_textbox_2">显示这个例子</A></P>
<P>在下面的例子中，我们在一个 .aspx 文件中声明了一个 TextBox 控件和一个 Label 控件。当您更改了 TextBox 中的值，并且在 
TextBox 外单击时，change 子例程就会被执行。change 子例程会向 Label 控件写一条文本：</P><PRE>&lt;script runat="server"&gt;
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P class=tiy><A href="../tiy/s.asp@f=demo_aspnet_textbox_3">显示这个例子</A></P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms ViewState" 
  href="aspnet_viewstate.asp">WebForms ViewState</A> </LI>
