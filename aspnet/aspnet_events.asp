...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP.NET - 事件</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="WebForms 事件" href="aspnet_events.asp">WebForms 
  事件</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms 控件" href="aspnet_controls.asp">WebForms 
  控件</A> </LI>
  <LI class=next><A title="WebForms 窗体" href="aspnet_forms.asp">WebForms 窗体</A> 
  </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>事件句柄（event handler）是一种针对给定事件来执行代码的子例程。</STRONG></P></DIV>
<DIV>
<H2>ASP.NET - 事件句柄</H2>
<P>请看下面的代码：</P><PRE>&lt;%
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P>上面的代码什么时候会被执行？答案是：“我不知道. . .”</P></DIV>
<DIV>
<H2>Page_Load 事件</H2>
<P>Page_Load 事件是众多 ASP.NET 可理解的事件之一。Page_Load 事件会在页面加载时被触发，然后 ASP.NET 会自动调用子例程 
Page_Load，并执行其中的代码：</P><PRE>&lt;script runat="server"&gt;
Sub Page_Load
lbl1.Text="The date and time is " &amp; now()
End Sub
&lt;/script&gt;
...
<P class=tiy><A href="../tiy/s.asp@f=demo_aspnet_pageload">显示这个例子</A></P></DIV>
<DIV>
<H2>Page.IsPostBack 属性</H2>
<P>Page_Load 子例程会在页面每次加载时运行。如果您仅希望在页面第一次加载时执行 Page_Load 子例程中的代码，您可以使用 
Page.IsPostBack 属性。如果 Page.IsPostBack 属性为 false，则页面第一次被载入，如果为 
true，则页面传回服务器（例如，通过点击表单上的按钮）：</P><PRE>&lt;script runat="server"&gt;
Sub Page_Load
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P>上面的例子仅在页面初次加载时创建 "The date and time is...." 这条消息。当用户点击 Submit 按钮时，submit 
子例程将在第二个 label 创建 "Hello World!"，但第一个 label 中的日期和时间不会改变。</P>
<P class=tiy><A 
href="../tiy/s.asp@f=demo_aspnet_pageispostback">显示这个例子</A></P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms 控件" href="aspnet_controls.asp">WebForms 
  控件</A> </LI>
