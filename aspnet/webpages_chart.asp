...
<HTML lang=zh-cn><HEAD><TITLE>ASP.NET Web Pages - Chart 帮助器</TITLE>
<META charset=gb2312>
...
<UL>
  ...
  WebGrid</A> 
  <LI class=currentLink><A title="WebPages 图表" 
  href="webpages_chart.asp">WebPages 图表</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebPages WebGrid" href="webpages_webgrid.asp">WebPages 
  WebGrid</A> </LI>
  <LI class=next><A title="WebPages 电邮" href="webpages_email.asp">WebPages 
  电邮</A> </LI></UL></DIV>
<DIV id=intro>
<P>Chart - 众多有用的 ASP.NET Web 帮助器之一。</P></DIV>
<DIV>
<H2>Chart Helper</H2>
...
<P>“Chart 
帮助器”能够通过多种格式和标记创建不同类型的图表图片。它能够创建标准图表，比如面积图、条状图、柱形图、线图和饼图，以及更专业的图表（比如股票走势图）。</P>
<P>显示在图表中的数据可以是来自数组、数据库或文件中的数据。</P></DIV>
<DIV>
<H2>来自数组的图表</H2>
...
<P>使用 DataBindTable 方法的替代方案是使用 AddSeries（参见前一个例子）。DataBindTable 更易使用，但是 
AddSeries 的伸缩性更强，因为它能够更精确地规定图表和数据：</P>
<H3>实例</H3><PRE>@{ 
var db = Database.Open("SmallBakery"); 
var dbdata = db.Query("SELECT Name, Price FROM Product"); 
var myChart = new Chart(width: 600, height: 400) 
...
<H3>实例</H3><PRE>@using System.Data;
...
<UL class=prenext>
  ...
  WebGrid</A> </LI>
  <LI class=next><A title="WebPages 电邮" href="webpages_email.asp">WebPages 
  电邮</A> </LI></UL></DIV></DIV><!-- maincontent end -->
<DIV id=sidebar>
<DIV id=searchui>
