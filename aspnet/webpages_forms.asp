...
<HTML lang=zh-cn><HEAD><TITLE>ASP.NET Web Pages - HTML 窗体</TITLE>
<META charset=gb2312>
...
<UL>
  ...
  <LI class=currentLink><A title="WebPages 窗体" 
  href="webpages_forms.asp">WebPages 窗体</A> 
...
<DIV id=maincontent>
<H1>ASP.NET Web Pages - HTML 窗体</H1>
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebPages 全局" href="webpages_global.asp">WebPages 
  全局</A> </LI>
  <LI class=next><A title="WebPages 对象" href="webpages_objects.asp">WebPages 
  对象</A> </LI></UL></DIV>
<DIV id=intro>
<P>窗体（表单）是您在 HTML 文档中放置输入控件（文本框、复选框、单选框和下拉菜单）的位置。</P></DIV>
<DIV>
<H2>创建 HTML 输入页面</H2>
<H3>Razor 实例</H3><PRE>&lt;html&gt;
&lt;body&gt; 
@{
if (IsPost) { 
...
<DIV>
<H2>Razor 实例 - 显示图像</H2>
<P>假设在您的图片文件夹中有三张图片，并且您希望根据用户的选择来动态地显示这些图片。</P>
<P>通过一小段 Razor 就可以很容易地实现。</P>
<P>如果网站图片文件夹中的图片的名称是 "Photo1.jpg"，则您可以使用 HTML &lt;img&gt; 元素来显示这幅图片，就像这样：</P><PRE>&lt;img src="images/Photo1.jpg" alt="Sample" /&gt;</PRE>
<P>下面的例子展示了如何显示用户从下拉列表中选择的图片：</P>
<H3>Razor 实例</H3><PRE>@{
var imagePath=""; 
...
<P>运行实例</P>
<H3>例子解释</H3>
<P>服务器创建名为 <B>imagePath</B> 的变量。</P>
<P>HTML 页面中有名为 <B>Choice</B> 的<B>下拉列表</B>（&lt;select&gt; 
元素）。它允许用户选择一个友好的（编者注：容易阅读的）名称（比如 <B>Photo 1</B>），当页面被提交到 web 服务器时，向服务器传递文件名（比如 
Photo1.jpg）。</P>
<P>Razor 通过 <B>Request["Choice"]</B> 读取 Choice 的值。如果值存在，则代码构造出指向图片的路径 
(images/Photo1.jpg)，并把路径存储于变量 <B>imagePath</B> 中。</P>
<P>HTML 页面中的 &lt;img&gt; 元素显示出该图片。当显示页面时，src 属性会被设置为 imagePath 变量的值。</P>
<P>如果变量 imagePath 的值为空，则会防止 &lt;img&gt; 元素显示不存在的图片（比如当页面首次加载时）。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebPages 全局" href="webpages_global.asp">WebPages 
  全局</A> </LI>
  <LI class=next><A title="WebPages 对象" href="webpages_objects.asp">WebPages 
  对象</A> </LI></UL></DIV></DIV><!-- maincontent end -->
<DIV id=sidebar>
<DIV id=searchui>
