...
<HTML lang=zh-cn><HEAD><TITLE>ASP.NET Web Pages - 文件夹</TITLE>
<META charset=gb2312>
...
<UL>
  ...
  <LI class=currentLink><A title="WebPages 文件夹" 
  href="webpages_folders.asp">WebPages 文件夹</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebPages 布局" href="webpages_layout.asp">WebPages 
  布局</A> </LI>
  <LI class=next><A title="WebPages 全局" href="webpages_global.asp">WebPages 
  全局</A> </LI></UL></DIV>
<DIV id=intro>
<P>本章介绍文件夹和文件夹路径。</P></DIV>
<DIV>
<P>在本章中，您将学到：</P>
<UL>
  <LI>逻辑和物理文件夹结构 
  <LI>虚拟和物理名称 
  <LI>web URL 和路径 </LI></UL></DIV>
<DIV>
<H2>逻辑文件夹结构</H2>
<P>下面是 ASP.NET 网页的典型文件夹结构：</P><IMG alt="Web Pages 文件夹结构" 
src="../i/webpages_folders.jpg"> 
<UL>
  <LI>"Account" 文件夹包含登录和安全文件 
  <LI>"App_Data" 文件包含数据库和数据文件 
  <LI>"Images" 文件夹包含图片 
  <LI>"Scripts" 文件夹包含浏览器脚本 
  <LI>"Shared" 文件夹包含通用文件（比如布局和样式文件） </LI></UL></DIV>
<DIV>
<H2>物理文件结构</H2>
<P>上面这个网站的 "Images" 文件夹的物理结构在计算机上类似这样：</P><PRE>C:\Johnny\Documents\MyWebSites\Demo\Images</PRE></DIV>
<DIV>
<H2>虚拟和物理名称</H2>
<P>根据上面的例子：</P>
<P>web 图片的虚拟名称类似这样："Images/pic31.jpg"。</P>
<P>但是物理名称类似这样 "C:\Johnny\Documents\MyWebSites\Demo\Images\pic31.jpg"。</P></DIV>
<DIV>
<H2>URL 和路径</H2>
<P>URL 用于从 web 访问文件：</P><PRE>http://www.w3school.com.cn/html5/html5_intro.asp</PRE>
<P>URL 在服务器上对应的文件是：</P><PRE>C:\MyWebSites\htm5\html5_intro.asp</PRE>
<P>虚拟路径是代表物理路径的简写形式。如果您使用虚拟路径，就能够在不更新路径的情况下将网页移动到不同的域名（或服务器）。</P><PRE>URL	http://www.w3school.com.cn/html/html5_intro.asp
服务器名	w3school
...
<P>ASP.NET 提供三种处理文件夹路径的工具：~ 操作符、Server.MapPath 方法以及 Href 方法。</P></DIV>
<DIV>
<H2>~ 操作符</H2>
<P>如需在编程代码中规定虚拟根目录，请使用 ~ 操作符。</P>
<P>如果您使用 ~ 操作符，而不是路径，就能够在不改变任何代码的情况下，将网站移动到不同的文件夹或位置：</P><PRE>var myImagesFolder = "~/images";
var myStyleSheet = "~/styles/StyleSheet.css";
</PRE></DIV>
<DIV>
<H2>Server.MapPath 方法</H2>
<P>Server.MapPath 方法将虚拟路径 (/default.cshtml) 转换为服务器能够理解的物理路径 
(C:\Johnny\MyWebSited\Demo\default.cshtml)。</P>
<P>当需要打开位于服务器上的数据文件时，您就会用到该方法（数据文件只能通过完整的物理路径来访问）：</P><PRE>var pathName = "~/dataFile.txt";
var fileName = Server.MapPath(pathName);
</PRE>
<P>在本教程的下一章中，您将学习更多有关在服务器上读取和写入数据文件的知识。</P></DIV>
<DIV>
<H2>Href 方法</H2>
<P>Href 方法把代码中的路径转换为浏览器能够理解的路径（浏览器无法理解 ~ 操作符）。</P>
<P>您使用 Href 方法来创建指向资源（比如图片和 CSS 文件）的路径。</P>
<P>您会经常在 HTML &lt;a&gt;、&lt;img&gt; 以及 &lt;link&gt; 元素中用到该方法：</P><PRE>@{var myStyleSheet = "~/Shared/Site.css";}
...
<P>Href 方法属于 WebPage 对象的方法。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebPages 布局" href="webpages_layout.asp">WebPages 
  布局</A> </LI>
  <LI class=next><A title="WebPages 全局" href="webpages_global.asp">WebPages 
  全局</A> </LI></UL></DIV></DIV><!-- maincontent end -->
<DIV id=sidebar>
<DIV id=searchui>
