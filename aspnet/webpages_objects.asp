...
<HTML lang=zh-cn><HEAD><TITLE>ASP.NET Web Pages - 对象</TITLE>
<META charset=gb2312>
...
<UL>
  ...
  <LI class=currentLink><A title="WebPages 对象" 
  href="webpages_objects.asp">WebPages 对象</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebPages 窗体" href="webpages_forms.asp">WebPages 窗体</A> 
  </LI>
  <LI class=next><A title="WebPages 文件" href="webpages_files.asp">WebPages 
  文件</A> </LI></UL></DIV>
<DIV id=intro>
<P>Web Pages 通常与对象相关。</P></DIV>
<DIV>
<H2>Page 对象</H2>
<P>您已经见到过一些使用过的 Page 对象：</P><PRE>@RenderPage("header.cshtml")

@RenderBody()
</PRE>
<P>在前一章，您已看到两个用过的 Page 对象属性（isPost 和 Request）：</P><PRE>If (isPost) {
...
<TABLE class=dataintable>
  ...
  <TR>
    <TD>href</TD>
    <TD>使用指定的参数构建 URL。</TD></TR>
  <TR>
    <TD>RenderBody()</TD>
    <TD>在布局页中，将呈现不在指定部分中的内容页部分。</TD></TR>
  <TR>
    <TD>RenderPage(page)</TD>
    <TD>在其他页内呈现某一页的内容。</TD></TR>
  <TR>
    <TD>RenderSection(section)</TD>
    <TD>在布局页中，将呈现指定部分的内容。</TD></TR>
  <TR>
    <TD>Write(object)</TD>
    <TD>将指定的对象作为 HTML 编码的字符串写入。</TD></TR>
  <TR>
    <TD>WriteLiteral</TD>
    <TD>无需先对指定的对象进行 HTML 编码，即可将其写入。</TD></TR></TBODY></TABLE></DIV>
<DIV>
<H2>Page 对象的一些属性</H2>
<TABLE class=dataintable>
  <TBODY>
  ...
  <TR>
    <TD>isPost</TD>
    <TD>返回一个值（true 或 false），指示客户端用来请求网页的 HTTP 数据传输方法是否为 POST 请求。</TD></TR>
  <TR>
    <TD>Layout</TD>
    <TD>获取或设置布局页的路径。</TD></TR>
  <TR>
    <TD>Page</TD>
    <TD>提供对页面、布局页和分页之间共享的页数据的类似属性的访问。</TD></TR>
  <TR>
    <TD>Request</TD>
    <TD>获得当前 HTTP 请求的 HttpRequest 对象。</TD></TR>
  <TR>
    <TD>Server</TD>
    <TD>获取 HttpServerUtility 
对象，该对象所提供的方法能够在网页处理过程中使用。</TD></TR></TBODY></TABLE></DIV>
<DIV>
...
<UL>
  <LI>Page.Title 
  <LI>Page.Version 
  <LI>Page.anythingyoulike </LI></UL>
<P>Page 属性很有用。举例，可以在内容文件中设置页面标题，然后在布局文件中使用它：</P>
<H3>Home.cshtml</H3><PRE>@{
...
<H3>Layout.cshtml</H3><PRE>&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<UL class=prenext>
  ...
  <LI class=next><A title="WebPages 文件" href="webpages_files.asp">WebPages 
  文件</A> </LI></UL></DIV></DIV><!-- maincontent end -->
<DIV id=sidebar>
<DIV id=searchui>
