...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP.NET - Hashtable 对象</TITLE>
...
<UL>
  ...
  ArrayList</A> 
  <LI class=currentLink><A title="WebForms Hashtable" 
  href="aspnet_hashtable.asp">WebForms Hashtable</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms ArrayList" 
  href="aspnet_arraylist.asp">WebForms ArrayList</A> </LI>
  <LI class=next><A title="WebForms SortedList" 
  href="aspnet_sortedlist.asp">WebForms SortedList</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>Hashtable 对象包含用键/值对表示的项目。</STRONG></P></DIV>
<DIV class=example>
<H2>实例</H2>
<DL>
  <DT><A href="../tiy/s.asp@f=demo_aspnet_hashtable_radio_1">Hashtable 
  RadiobuttonList 1</A> 
  <DT><A href="../tiy/s.asp@f=demo_aspnet_hashtable_radio_2">Hashtable 
  RadiobuttonList 2</A> 
  <DT><A href="../tiy/s.asp@f=demo_aspnet_hashtable_drop_1">Hashtable 
  DropDownList</A> </DT></DL></DIV>
<DIV>
<H2>创建 Hashtable</H2>
<P>Hashtable 对象包含用键/值对表示的项目。键被用作索引，通过搜索其键，可以实现对值的快速搜索。</P>
<P>通过 Add() 方法向 Hashtable 添加项目。</P>
<P>下面的代码创建一个名为 mycountries 的 Hashtable，并添加了四个元素：</P><PRE>&lt;script runat="server"&gt;
Sub Page_Load
...
<P>如需把数据绑定到某个 RadioButtonList 控件，首先请在一个 .aspx 页面中创建 RadioButtonList 控件（没有任何 
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P>然后添加构建列表的脚本：</P><PRE>&lt;script runat="server"&gt;
sub Page_Load
if Not Page.IsPostBack then
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P>然后我们添加一个子例程，该例程会在用户点击 RadioButtonList 控件中的某个项目时被执行。当某个单选按钮被点击，label 
中会出现一条文本：</P><PRE>&lt;script runat="server"&gt;
sub Page_Load
if Not Page.IsPostBack then
  dim mycountries=New Hashtable
  mycountries.Add("C","China")
...
&lt;/html&gt;</PRE>
<P class=tiy><A 
href="../tiy/s.asp@f=demo_aspnet_hashtable_radio_1">显示这个例子</A></P>
<P class=note><SPAN>注释：</SPAN>您无法选择添加到 Hashtable 的项目的排序方式。如需对项目进行字母排序或数字排序，请使用 
SortedList 对象。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms ArrayList" 
  href="aspnet_arraylist.asp">WebForms ArrayList</A> </LI>
  <LI class=next><A title="WebForms SortedList" 
  href="aspnet_sortedlist.asp">WebForms SortedList</A> </LI></UL></DIV></DIV>
<DIV id=sidebar>
<DIV id=searchui>
