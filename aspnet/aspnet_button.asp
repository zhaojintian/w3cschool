...
<UL>
  ...
  <LI class=currentLink><A title="WebForms Button" 
  href="aspnet_button.asp">WebForms Button</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms TextBox" href="aspnet_textbox.asp">WebForms 
  TextBox</A> </LI>
  <LI class=next><A title="WebForms Data Binding" 
  href="aspnet_databinding.asp">WebForms 数据绑定</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>Button 控件用于显示一个按钮。</STRONG></P></DIV>
<DIV>
<H2>Button 控件</H2>
<P>Button 控件用于显示按钮。按钮可以是提交按钮或命令按钮，而该控件属于提交按钮。</P>
<P>submit 按钮没有命令名称，当它被点击时，它把页面传回服务器。可以编写一些事件句柄，在提交按钮被点击时来控制动作的执行。</P>
<P>command 按钮拥有命名名称，且允许你在页面上创建多个 Button 控件。可以编写一些事件句柄，在 command 
按钮被点击时来控制动作的执行。</P>
<P>Button 控件的属性列在我们的 <A title="ASP.NET Button 控件" 
href="control_button.asp">Button 控件参考手册</A>中。</P>
<P>下面的例子演示了一个简单的 Button 控件：</P><PRE>&lt;html&gt;
&lt;body&gt;
...
<H2>添加脚本</H2>
<P>表单通常通过点击按钮进行提交。</P>
<P>在下面的例子中，我们在 .aspx 文件中声明了一个 TextBox 控件、一个 Button 控件以及一个 Label 
控件。当这个提交按钮被点击时，submit 子例程就会被执行。该 submit 子例程会向 Label 控件写一条文本：</P><PRE>&lt;script runat="server"&gt;
...
&lt;/body&gt;
&lt;/html&gt;</PRE>
<P class=tiy><A href="../tiy/s.asp@f=demo_aspnet_textbox_2">显示这个例子</A></P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms TextBox" href="aspnet_textbox.asp">WebForms 
  TextBox</A> </LI>
  <LI class=next><A title="WebForms Data Binding" 
  href="aspnet_databinding.asp">WebForms 数据绑定</A> </LI></UL></DIV></DIV>
<DIV id=sidebar>
<DIV id=searchui>
