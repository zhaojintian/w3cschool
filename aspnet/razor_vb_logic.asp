...
<HTML lang=zh-cn><HEAD><TITLE>ASP.NET Razor - VB 逻辑条件</TITLE>
<META charset=gb2312>
...
<UL>
  ...
  <LI class=currentLink><A title="Razor VB 逻辑" href="razor_vb_logic.asp">Razor 
  VB 逻辑</A> </LI></UL>
<H2>ASP.NET MVC</H2>
<UL>
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="Razor VB 循环" href="razor_vb_loops.asp">Razor VB 循环</A> 
  </LI>
  <LI class=next><A title="MVC 简介" href="mvc_intro.asp">MVC 简介</A> 
</LI></UL></DIV>
<DIV id=intro>
...
<UL>
  <LI>if 语句启动代码块 
  <LI>条件位于 if 和 then 之间 
  <LI>如果条件为真，则执行 if ... then 与 end if 之间的代码 </LI></UL>
<H3>实例</H3><PRE>@Code
Dim price=50
End Code
&lt;html&gt;
...
<P>运行实例</P>
<P>"Select Case" 之后是测试值 (day)。每个具体的测试条件以 case 关键词开头，其后允许任意数量的代码行。如果测试值匹配 case 
值，则执行代码行。</P>
<P>select 代码块可为其余的情况设置默认的 case (default:)，允许在所有 case 均不为 true 时执行代码。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="Razor VB 循环" href="razor_vb_loops.asp">Razor VB 循环</A> 
  </LI>
