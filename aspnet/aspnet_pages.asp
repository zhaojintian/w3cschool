...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP.NET - Web 页面</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="WebForms Pages" 
  href="aspnet_pages.asp">WebForms Pages</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms 简介" href="aspnet_intro.asp">WebForms 简介</A> 
  </LI>
  <LI class=next><A title="WebForms 控件" href="aspnet_controls.asp">WebForms 
  控件</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>简单的 ASP.NET 页面看上去类似普通的 HTML 页面。</STRONG></P></DIV>
<DIV>
<H2>Hello W3School</H2>
<P>为了开始我们的 ASP.NET 学习之旅，首先我们将构造一张简单的 HTML 页面，这张页面将在浏览器中显示 "Hello W3School"：</P>
<DIV id=hellow3school 
style="BORDER-TOP: #999 1px solid; HEIGHT: 100px; BORDER-RIGHT: #999 1px solid; BORDER-BOTTOM: #999 1px solid; BORDER-LEFT: #999 1px solid; MARGIN-TOP: 15px; WIDTH: 550px; BACKGROUND-COLOR: #e5eecc">
<P style="FONT-SIZE: 18px; FONT-WEIGHT: bold; TEXT-ALIGN: center">Hello 
W3School</P></DIV></DIV>
<DIV>
<H2>用 HTML 编写的 Hello W3School</H2>
<P>这张 HTML 页面的 HTML 代码：</P><PRE>&lt;html&gt;
&lt;body style="background-color:#e5eecc; text-align:center;"&gt;
&lt;h2&gt;Hello W3School!&lt;/h2&gt;
&lt;/body&gt;
&lt;/html&gt;
</PRE>
<P>如果您希望亲自试一试，可以把这些代码保存在名为 "firstpage.html" 的文件中，然后创建一个到此文件的链接，就像这样：<A 
href="firstpage.html">firstpage.html</A>。</P></DIV>
<DIV>
<H2>用 ASP.NET 编写的 Hello W3School</H2>
<P>把 HTML 页面转换为 ASP.NET 的最简单的方法是，把这个 HTML 文件拷贝为带有 .aspx 后缀的新文件。</P>
<P>这些将把我们的例子显示为一个 ASP.NET 页面：</P><PRE>&lt;html&gt;
&lt;body style="background-color:#e5eecc; text-align:center;"&gt;
&lt;h2&gt;Hello W3School!&lt;/h2&gt;
&lt;/body&gt;
&lt;/html&gt;
</PRE>
<P>如果您希望亲自试一试，请把这些代码保存在一个名为 "firstpage.aspx" 的文件中，并创建一个到此文件的链接：<A 
href="firstpage.aspx">firstpage.aspx</A>。</P></DIV>
<DIV>
<H2>它如何工作？</H2>
<P>从根本上讲，ASP.NET 页面与 HTML 完全相同。</P>
<P>HTML 页面的扩展名是 .htm 或 .html。假如浏览器从服务器请求某张 HTML 页面，服务器不进行任何修改，就会把该页面发往浏览器。</P>
<P>ASP.NET 页面的扩展名是 .aspx。如果浏览器请求某张 ASP.NET 
页面，那么在把结果发回浏览器之前，服务器首先会处理页面中的可执行代码。</P>
<P>上面的 ASP.NET 页面不包含任何可执行的代码，因此也不会执行任何代码。在下面的例子中，我们将向页面添加一些可执行代码，以便向您演示静态 HTML 
页面与动态 ASP 页面的不同之处。</P></DIV>
<DIV>
<H2>经典的 ASP</H2>
...
<P>如果您希望学习更多有关 Classic ASP 的知识，请访问我们的 <A href="../asp/index.asp">ASP 
教程</A>。</P></DIV>
<DIV>
<H2>用 Classic ASP 编写的动态页面</H2>
<P>为了向您演示如何使用动态内容显示页面，我们向上面的例子添加了一些可执行代码：</P><PRE>&lt;html&gt;
&lt;body style="background-color:#e5eecc; text-align:center;"&gt;
&lt;h2&gt;Hello W3School!&lt;/h2&gt;
...
<P>如果您希望亲自试一试，可以把这些代码保存在名为 "dynpage.asp" 的文件中，并创建一个指向该文件的链接：<A 
href="dynpage.asp">dynpage.asp</A>。</P></DIV>
<DIV>
<H2>用 ASP .NET 编写的动态页面</H2>
<P>下面的代码可把我们的例子显示为一个 ASP.NET 页面：</P><PRE>&lt;html&gt;
&lt;body style="background-color:#e5eecc; text-align:center;"&gt;
&lt;h2&gt;Hello W3School!&lt;/h2&gt;
&lt;p&gt;<CODE>&lt;%Response.Write(now())%&gt;</CODE>&lt;/p&gt;
&lt;/body&gt;
...
<P>如果您希望亲自试一试，请把这些代码保存在名为 "dynpage.aspx" 的文件中，然后创建到此文件的链接：<A 
href="dynpage.aspx">dynpage.aspx</A>。</P></DIV>
<DIV>
...
<P>在下面的章节中，您会看到，服务器控件如何使 ASP.NET 比 Classic ASP 更加强大。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="WebForms 简介" href="aspnet_intro.asp">WebForms 简介</A> 
  </LI>
