...
<li class="currentLink"><a href="php_syntax.asp" title="PHP 语法">PHP 语法</a></li>
...
<div  id="tpn">
<ul class="prenext">
<li class="pre"><a href="php_install.asp" title="PHP 安装">PHP 安装</a></li>
<li class="next"><a href="php_variables.asp" title="PHP 变量">PHP 变量</a></li>
</ul>
</div>

<div id="intro">
<p><strong>PHP 脚本在服务器上执行，然后向浏览器发送回纯 HTML 结果。</strong></p>
</div>
...
<p>PHP 脚本可放置于文档中的任何位置。</p>

<p>PHP 脚本以 <em>&lt;?php</em> 开头，以 <em>?&gt;</em> 结尾：</p>

<pre>
&lt;?php
<span class="code_comment">// 此处是 PHP 代码</span>
?&gt;
</pre>

<p>PHP 文件的默认文件扩展名是 &quot;.php&quot;。</p>

<p>PHP 文件通常包含 HTML 标签以及一些 PHP 脚本代码。</p>

<p>下面的例子是一个简单的 PHP 文件，其中包含了使用内建 PHP 函数 &quot;echo&quot; 在网页上输出文本 &quot;Hello World!&quot; 的一段 PHP 脚本：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_syntax">运行实例</a></p>

<p class="note"><span>注释：</span>PHP 语句以分号结尾（;）。PHP 代码块的关闭标签也会自动表明分号（因此在 PHP 代码块的最后一行不必使用分号）。</p>
</div>
...
<p>PHP 代码中的注释不会被作为程序来读取和执行。它唯一的作用是供代码编辑者阅读。</p>

<p>注释用于：</p>

<ul>
<li>使其他人理解您正在做的工作 - 注释可以让其他程序员了解您在每个步骤进行的工作（如果您供职于团队）</li>
<li>提醒自己做过什么 - 大多数程序员都曾经历过一两年后对项目进行返工，然后不得不重新考虑他们做过的事情。注释可以记录您在写代码时的思路。</li>
</ul>
</div>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_syntax_comments">运行实例</a></p>
</div>
...
<p>在 PHP 中，所有用户定义的函数、类和关键词（例如 if、else、echo 等等）都对大小写不敏感。</p>

<p>在下面的例子中，所有这三天 echo 语句都是合法的（等价）：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_syntax_case1">运行实例</a></p>

<p>不过在 PHP 中，所有变量都对大小写敏感。</p>

<p>在下面的例子中，只有第一条语句会显示 $color 变量的值（这是因为 $color、$COLOR 以及 $coLOR 被视作三个不同的变量）：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_syntax_case2">运行实例</a></p>
</div>
