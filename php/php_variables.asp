...
<li class="currentLink"><a href="php_variables.asp" title="PHP 变量">PHP 变量</a></li>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="php_syntax.asp" title="PHP 语法">PHP 语法</a></li>
<li class="next"><a href="php_echo_print.asp" title="PHP Echo 和 Print 语句">PHP Echo / Print</a></li>
</ul>
</div>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_var1">运行实例</a></p>
</div>
...
<p>在代数中我们使用字母（比如 x）来保存值（比如 5）。</p>

<p>从上面的表达式 z=x+y，我们能够计算出 z 的值是 11。</p>

<p>在 PHP 中，这三个字母被称为<em>变量</em>。</p>

<p class="note"><span>注释：</span>请把变量视为存储数据的容器。</p>
</div>
...
<p>正如代数，PHP 变量可用于保存值（x=5）和表达式（z=x+y）。</p>

<p>变量的名称可以很短（比如 x 和 y），也可以取更具描述性的名称（比如 carname、total_volume）。</p>
...
<p class="note"><span>注释：</span>PHP 变量名称对大小写敏感！</p>
</div>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_var2">运行实例</a></p>

<p>以上语句执行后，变量 txt 会保存值 Hello world!，变量 x 会保存值 5，变量 y 会保存值 10.5。</p>

<p class="note"><span>注释：</span>如果您为变量赋的值是文本，请用引号包围该值。</p>
</div>


<div>
<h2>PHP 是一门类型松散的语言</h2>

<p>在上面的例子中，请注意我们不必告知 PHP 变量的数据类型。</p>

<p>PHP 根据它的值，自动把变量转换为正确的数据类型。</p>

<p>在诸如 C 和 C++ 以及 Java 之类的语言中，程序员必须在使用变量之前声明它的名称和类型。</p>
</div>
...
<div>
<h2>Local 和 Global 作用域</h2>

<p>函数<em>之外</em>声明的变量拥有 Global 作用域，只能在函数以外进行访问。</p>

<p>函数<em>内部</em>声明的变量拥有 LOCAL 作用域，只能在函数内部进行访问。</p>

<p>下面的例子测试了带有局部和全局作用域的变量：</p>

<h3>实例</h3>

<pre>
&lt;?php
$x=5; <span class="code_comment">// 全局作用域</span>

function myTest() {
  $y=10; <span class="code_comment">// 局部作用域</span>
  echo &quot;&lt;p&gt;测试函数内部的变量：&lt;/p&gt;&quot;;
  echo &quot;变量 x 是：$x&quot;;
  echo &quot;&lt;br&gt;&quot;;
  echo &quot;变量 y 是：$x&quot;;
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_var_local">运行实例</a></p>
...
<p class="note"><span>注释：</span>您可以在不同的函数中创建名称相同的局部变量，因为局部变量只能被在其中创建它的函数识别。</p>
</div>


<div>
<h2>PHP global 关键词</h2>
...
myTest();
echo $y; <span class="code_comment">// 输出 15</span>
?&gt;
</pre>

<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_var_global">运行实例</a></p>

<p>PHP 同时在名为 $GLOBALS[index] 的数组中存储了所有的全局变量。下标存有变量名。这个数组在函数内也可以访问，并能够用于直接更新全局变量。</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_var_global2">运行实例</a></p>
</div>


<div>
<h2>PHP static 关键词</h2>

<p>通常，当函数完成/执行后，会删除所有变量。不过，有时我需要不删除某个局部变量。实现这一点需要更进一步的工作。</p>

<p>要完成这一点，请在您首次声明变量时使用 <em>static</em> 关键词：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_var_static">运行实例</a></p>

<p>然后，每当函数被调用时，这个变量所存储的信息都是函数最后一次被调用时所包含的信息。</p>

<p class="note"><span>注释：</span>该变量仍然是函数的局部变量。</p>
</div>
