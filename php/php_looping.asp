...
<li class="currentLink"><a href="php_looping.asp" title="PHP while 循环">PHP While 循环</a></li>
...
<div  id="tpn">
<ul class="prenext">
<li class="pre"><a href="php_switch.asp" title="PHP Switch 语句">PHP Switch</a></li>
<li class="next"><a href="php_looping_for.asp" title="PHP for 循环">PHP For 循环</a></li>
</ul>
</div>


<div id="intro">
<p><strong>PHP while 循环在指定条件为 true 时执行代码块。</strong></p>
</div>
...
<p>在您编写代码时，经常需要反复运行同一代码块。我们可以使用循环来执行这样的任务，而不是在脚本中添加若干几乎相等的代码行。</p>

<p>在 PHP 中，我们有以下循环语句：</p>

<ul>
<li><em>while</em> - 只要指定条件为真，则循环代码块</li>
<li><em>do...while</em> - 先执行一次代码块，然后只要指定条件为真则重复循环</li>
<li><em>for</em> - 循环代码块指定次数</li>
<li><em>foreach</em> - 遍历数组中的每个元素并循环代码块</li>
</ul>
</div>
...
<p>上例首先把变量 $x 设置为 1（$x=1）。然后执行 while 循环，只要 $x 小于或等于 5。循环每运行一次，$x 将递增 1：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_loop_while">运行实例</a></p>
</div>


<div>
<h2>PHP do...while 循环</h2>

<p>do...while 循环首先会执行一次代码块，然后检查条件，如果指定条件为真，则重复循环。</p>
...
<p>下面的例子首先把变量 $x 设置为 1（$x=1）。然后，do while 循环输出一段字符串，然后对变量 $x 递增 1。随后对条件进行检查（$x 是否小于或等于 5）。只要 $x 小于或等于 5，循环将会继续运行：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_loop_do_while">运行实例</a></p>

<p>请注意，do while 循环只在执行循环内的语句之后才对条件进行测试。这意味着 do while 循环至少会执行一次语句，即使条件测试在第一次就失败了。</p>

<p>下面的例子把 $x 设置为 6，然后运行循环，<em>随后对条件进行检查</em>：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_loop_do_while2">运行实例</a></p>

<p>下一节会讲解 for 循环和 foreach 循环。</p>
</div>
