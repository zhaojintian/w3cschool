...
<li class="currentLink"><a href="php_includes.asp" title="PHP Include 文件">PHP Include</a></li>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="php_date.asp" title="PHP Date()">PHP 日期</a></li>
<li class="next"><a href="php_file.asp" title="PHP 文件处理">PHP 文件</a></li>
</ul>
</div>
...
<p class="marked">include 和 require 语句是相同的，除了错误处理方面：</p>
...
<div>
<h2>PHP include 实例</h2>

<h3>例子 1</h3>

<p>假设我们有一个名为 &quot;footer.php&quot; 的标准的页脚文件，就像这样：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_include1">运行实例</a></p>


<h3>例子 2</h3>

<p>假设我们有一个名为 &quot;menu.php&quot; 的标准菜单文件：</p>
...
<p>网站中的所有页面均使用此菜单文件。具体的做法是（我们使用了一个 &lt;div&gt; 元素，这样今后就可以轻松地通过 CSS 设置样式）：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_include2">运行实例</a></p>


<h3>例子 3</h3>

<p>假设我们有一个名为 &quot;vars.php&quot; 的文件，其中定义了一些变量：</p>
...
<p>然后，如果我们引用这个 &quot;vars.php&quot; 文件，就可以在调用文件中使用这些变量：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_include3">运行实例</a></p>
</div>
...
<p>require 语句同样用于向 PHP 代码中引用文件。</p>

<p>不过，include 与 require 有一个巨大的差异：如果用 include 语句引用某个文件并且 PHP 无法找到它，脚本会继续执行：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_include4">运行实例</a></p>

<p>如果我们使用 require 语句完成相同的案例，echo 语句不会继续执行，因为在 require 语句返回严重错误之后脚本就会终止执行：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_include5">运行实例</a></p>
</div>
...
<p>请在此时使用 require：当文件被应用程序请求时。</p>

<p>请在此时使用 include：当文件不是必需的，且应用程序在文件未找到时应该继续运行时。</p>
</div>
