...
<li class="currentLink"><a href="php_functions.asp" title="PHP 函数">PHP 函数</a></li>
...
<div  id="tpn">
<ul class="prenext">
<li class="pre"><a href="php_looping_for.asp" title="PHP for 循环">PHP For 循环</a></li>
<li class="next"><a href="php_arrays.asp" title="PHP 数组">PHP 数组</a></li>
</ul>
</div>


<div id="intro">
<p><strong>PHP 的真正力量来自它的函数：它拥有超过 1000 个内建的函数。</strong></p>
</div>
...
<div>
<h2>在 PHP 创建用户定义函数</h2>

<p>用户定义的函数声明以关单 &quot;function&quot; 开头：</p>
...
<p class="note"><span>注释：</span>函数名能够以字母或下划线开头（而非数字）。</p>

<p class="note"><span>注释：</span>函数名对大小写不敏感。</p>

<p class="tip"><span>提示：</span>函数名应该能够反映函数所执行的任务。</p>
...
writeMsg(); <span class="code_comment">// 调用函数</span>
?&gt;
</pre>

<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_function1">运行实例</a></p>
</div>
...
<p>可以通过参数向函数传递信息。参数类似变量。</p>

<p>参数被定义在函数名之后，括号内部。您可以添加任意多参数，只要用逗号隔开即可。</p>

<p>下面的例子中的函数有一个参数（$fname）。当调用 familyName() 函数时，我们同时要传递一个名字（例如 Bill），这样会输出不同的名字，但是姓氏相同：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_function2">运行实例</a></p>

<p>下面的例子中的函数有两个参数（$fname 和 $year）：</p>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_function3">运行实例</a></p>
</div>
...
<p>下面的例子展示了如何使用默认参数。如果我们调用没有参数的 setHeight() 函数，它的参数会取默认值：</p>
...
setHeight(350);
setHeight(); <span class="code_comment">// 将使用默认值 50</span>
setHeight(135);
setHeight(80);
?&gt;
</pre>

<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_function4">运行实例</a></p>
</div>
...
<p class="tiy"><a  href="../tiy/s.asp@f=demo_php_function5">运行实例</a></p>
</div>
