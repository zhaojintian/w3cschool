...
<HTML lang=zh-cn><HEAD><TITLE>PHP arsort() 函数</TITLE>
<META charset=gbk>
...
<H3>语法</H3><PRE>arsort(array,sorttype)</PRE>
  ...
  <TR>
    ...
    <TD>
      ...
      <UL class=listintable>
        <LI>SORT_REGULAR - 默认。以它们原来的类型进行处理（不改变类型）。 
        <LI>SORT_NUMERIC - 把值作为数字来处理 
        <LI>SORT_STRING - 把值作为字符串来处理 
        <LI>SORT_LOCALE_STRING - 把值作为字符串来处理，基于本地设置<SUP 
        style="FONT-FAMILY: Verdana, Arial, Helvetica, sans-serif">*</SUP>。 
      </LI></UL></TD></TR></TBODY></TABLE>
<P><SUP>*</SUP>：该值是 PHP 4.4.0 和 5.0.2 新加的。在 PHP 6 之前，使用了系统的区域设置，可以用 setlocale() 
来改变。自 PHP 6 起，必须用 i18n_loc_set_default() 函数。</P></DIV>
<DIV>
<H2>例子</H2><PRE>&lt;?php
