...
<HTML lang=zh-cn><HEAD><TITLE>PHP array()</TITLE>
<META charset=gbk>
...
<P><A title="PHP Array 函数" href="php_ref_array.asp">PHP Array 函数</A></P></DIV>
<DIV>
<H2>定义和用法</H2>
<P>array() 创建数组，带有键和值。如果在规定数组时省略了键，则生成一个整数键，这个 key 从 0 开始，然后以 1 进行递增。</P>
<P>要用 array() 创建一个关联数组，可使用 =&gt; 来分隔键和值。</P>
<P>要创建一个空数组，则不传递参数给 array()：</P><PRE>$new = array();</PRE>
<P class=important><SPAN>注意：</SPAN>array() 实际上是一种语言结构 (language 
construct)，通常用来定义直接量数组，但它的用法和函数的用法很相似，所以我们把它也列到手册中。</P>
<H3>语法</H3><PRE>array(key =&gt; value) </PRE>
<TABLE class=dataintable>
  ...
  <TR>
    <TD>key</TD>
    <TD>可选。规定 key，类型是数值或字符串。如果未设置，则生成整数类型的 key。</TD></TR>
  <TR>
    <TD>value</TD>
    <TD>必需。规定值。</TD></TR></TBODY></TABLE></DIV>
<DIV>
<H2>例子 1</H2><PRE>&lt;?php
$a=array("a"=&gt;"Dog","b"=&gt;"Cat","c"=&gt;"Horse");
print_r($a);
?&gt; </PRE>
<P>输出：</P><PRE>Array ( [a] =&gt; Dog [b] =&gt; Cat [c] =&gt; Horse )</PRE></DIV>
<DIV>
<H2>例子 2</H2><PRE>&lt;?php
$a=array("Dog","Cat","Horse");
print_r($a);
?&gt; </PRE>
<P>输出：</P><PRE>Array ( [0] =&gt; Dog [1] =&gt; Cat [2] =&gt; Horse )</PRE></DIV>
<DIV class=backtoreference>
<P><A title="PHP Array 函数" href="php_ref_array.asp">PHP Array 
