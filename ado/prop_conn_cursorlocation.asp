...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO CursorLocation 属性</TITLE>
...
<DIV>
<H2>定义和用法</H2>
<P>CursorLocation 属性可设置或返回一个 long 值，该值指示游标服务的位置。可被设置为 <A 
href="app_cursorlocationenum.asp">CursorLocationEnum</A> 值之一。默认值是 
AdUseServer。</P>
<P>游标用于：</P>
<UL>
  <LI>控制记录定位 
  <LI>控制其他用户对数据库所作的更改的可视性 
  <LI>控制数据可更新性 </LI></UL>
<P class=note><SPAN>注释：</SPAN>Recordset 对象将从与它们相关联的连接上自动继承此设置。</P>
<P class=note><SPAN>注释：</SPAN>此属性在 Connection 或已关闭的 Recordset 上为读/写，在打开的 
Recordset 上为只读。</P>
<H3>语法</H3><PRE>objConnection.CursorLocation
