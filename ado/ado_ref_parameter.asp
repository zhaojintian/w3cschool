...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO Parameter 对象</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="ADO Parameter 对象" 
  href="ado_ref_parameter.asp">ADO Parameter</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ADO Field 对象" href="ado_ref_field.asp">ADO Field</A> 
  </LI>
  <LI class=next><A title="ADO Property 对象" href="ado_ref_property.asp">ADO 
  Property</A> </LI></UL></DIV>
<DIV>
<H2>Parameter 对象</H2>
<P>ADO Parameter 对象可提供有关被用于存储过程或查询中的一个单个参数的信息。</P>
<P>Parameter 对象在其被创建时被添加到 Parameters 集合。Parameters 集合与一个具体的 Command 
对象相关联，Command 对象使用此集合在存储过程和查询内外传递参数。</P>
<P>参数被用来创建参数化的命令。这些命令（在它们已被定义和存储之后）使用参数在命令执行前来改变命令的某些细节。例如，SQL SELECT 语句可使用参数定义 
WHERE 子句的匹配条件，而使用另一个参数来定义 SORT BY 子句的列的名称。</P>
...
<TABLE class=dataintable>
  ...
  <TR>
    ...
    <TD>把长二进制或字符数据追加到一个 Parameter 对象。</TD></TR>
  <TR>
    <TD><A href="met_para_delete.asp">Delete</A></TD>
    <TD>从 Parameters 集合中删除一个对象。</TD></TR></TBODY></TABLE></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="ADO Field 对象" href="ado_ref_field.asp">ADO Field</A> 
  </LI>
  <LI class=next><A title="ADO Property 对象" href="ado_ref_property.asp">ADO 
  Property</A> </LI></UL></DIV></DIV>
<DIV id=sidebar>
<DIV id=searchui>
