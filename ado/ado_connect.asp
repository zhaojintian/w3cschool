...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO 数据库连接</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="ADO 数据库连接" href="ado_connect.asp">ADO 连接</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ADO 简介" href="ado_intro.asp">ADO 简介</A> </LI>
  <LI class=next><A title="ADO Recordset（记录集）" href="ado_recordset.asp">ADO 
  记录集</A> </LI></UL></DIV>
<DIV>
<P><STRONG>在从某个网页访问数据之前，必须先建立一个数据库连接。</STRONG></P></DIV>
<DIV>
<H2>创建一个 DSN-less 数据库连接</H2>
<P>连接到某一个数据库的最简单的方法是使用一个 DSN-less 连接。DSN-less 连接可被用于您的站点上的任何微软 Access 数据库。</P>
<P>假设您拥有一个名为 "northwind.mdb" 的数据库位于 "c:/webdata/" 的 web 目录中，您可以使用下面的 ASP 
代码连接到此数据库：</P><PRE>&lt;%
...
<P>注意，在上面的例子中，您必须规定微软的 Access 数据库驱动程序（Provider），以及此数据库在计算机上的物理路径。</P></DIV>
<DIV>
<H2>创建一个 ODBC 数据库连接</H2>
<P>假设您拥有一个名为 "northwind" 的 ODBC 数据库，您可以使用下面的 ASP 代码连接到此数据库：</P><PRE>&lt;%
set conn=Server.CreateObject("ADODB.Connection") 
conn.Open "northwind"
%&gt;
</PRE>
<P>通过一个 ODBC 连接，您可以连接到您的网络中任何计算机上的任何数据库，只要 ODBC 连接是可用的。</P></DIV>
<DIV>
<H2>到 MS Access 数据库的 ODBC 连接</H2>
<P>下面为您讲解如何创建到一个 MS Access 数据库的连接：</P>
<OL>
  <LI>打开控制面板中的 <EM>ODBC</EM> 图标 
  <LI>选择系统 <EM>ODBC 选项卡</EM> 
  <LI>点击 ODBC 选项卡中的<EM>添加</EM>按钮 
  <LI><EM>选择</EM> Driver to Microsoft Access，然后点击<EM>完成</EM>按钮 
  <LI>在下一个窗口中点击“选择”按钮来定位数据库 
  <LI>为此数据库赋予一个数据源名称（<EM>D</EM>ata <EM>S</EM>ource <EM>N</EM>ame，DSN） 
  <LI>点击"<EM>确定</EM>" </LI></OL>
<P 
class=note><SPAN>注意：</SPAN>此配置必须在您的网站所在的计算机上完成。假如您正在自己的计算机上运行PWS或者IIS，此架构是可以运行的，但是假如您的网站位于一台远程的服务器，您就必须拥有此服务器的物理访问权限，或者请您的 
web 主机提供商为您做这些事情。</P></DIV>
<DIV>
<H2>ADO 连接对象（ADO Connection Object）</H2>
<P>ADO 连接对象用来创建到某个数据源的开放连接。通过此连接，您可以对此数据库进行访问和操作。</P>
<P>查看此连接对象的所有方法和属性。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="ADO 简介" href="ado_intro.asp">ADO 简介</A> </LI>
  <LI class=next><A title="ADO Recordset（记录集）" href="ado_recordset.asp">ADO 
