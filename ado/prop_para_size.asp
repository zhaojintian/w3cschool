...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO Size 属性</TITLE>
...
<H3>语法</H3><PRE>objparameter.Size</PRE>
<H3>说明</H3>
<P>如果要指定 Parameter 对象的变长数据类型（例如所有 String 类型，如 adVarChar），必须先设置对象的 Size 
属性，然后再将该对象追加到 Parameters 集合。否则，将会产生错误。</P>
<P>如果已经将 Parameter 对象追加到 Command 对象的 Parameters 集合中，并将其类型更改为变长数据类型，则必须先设置 
Parameter 对象的 Size 属性，然后再执行 Command 对象。否则，将发生错误。</P>
<P>如果使用 Refresh 方法从提供者获取参数信息，并且返回一个或多个变长数据类型 Parameter 对象，ADO 
可能会根据其可能的最大尺寸为这些参数分配内存空间，这在执行过程中可能会导致错误。若要避免错误，在执行命令前应显式地设置这些参数的 Size 属性。</P>
<P>Size 属性为读/写。</P></DIV>
<DIV>
<H2>实例</H2><PRE>&lt;%
