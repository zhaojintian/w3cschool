...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO BeginTrans, CommitTran 以及 RollbackTrans 方法</TITLE>
...
<P class=note><SPAN>注释：</SPAN>BeginTrans、CommitTrans 和 RollbackTrans 方法在客户端 
Connection 对象上无效。</P>
<H3>BeginTrans</H3>
...
<H3>说明</H3>
<P>对于支持嵌套事务的提供者，在打开的事务中调用 BeginTrans 
方法将启动新的嵌套事务。返回值将指示嵌套层次：返回值为“1”指示打开了顶层事务（即此事务未嵌套在其他事务中），返回值为“2”指示打开了第二层事务（嵌套在顶层事务中的事务），依此类推。调用 
CommitTrans 或 RollbackTrans 只影响最新打开的事务；在处理任何更高层事务之前必须关闭或回卷当前事务。</P>
<H3>语法</H3><PRE>level=objconn.BeginTrans()
