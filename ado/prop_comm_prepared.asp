...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO Prepared 属性</TITLE>
...
<DIV>
<H2>定义和用法</H2>
<P>Prepared 属性可返回或设置一个布尔值，如果设置为 True，则指示该命令应该保存查询的一个准备好的（或已编译的）版本, 该查询是第一次执行 
Command 对象前在 CommandText 属性中指定的。</P>
<P>这会降低命令的首次执行速度，但是在第一次执行执行后，provider 将使用已编译的版本，这样就可以加快执行速度。</P>
<P>如果该属性为 False，提供者将直接执行 Command 对象而不创建编译版本。</P>
<P>如果提供者不支持命令准备，则一旦将此属性设置为 True，提供者便可能返回错误。如果它不返回错误，则仅忽略准备命令的请求，并将 Prepared 
属性设置为 False。</P>
<H3>语法</H3><PRE>objcommand.Prepared=true or false</PRE></DIV>
<DIV>
<H2>实例</H2><PRE>&lt;%
