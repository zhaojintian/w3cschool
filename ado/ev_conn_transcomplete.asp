...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO BeginTransComplete, CommitTransComplete 以及 RollbackTransComplete 事件</TITLE>
...
<UL>
  <LI>BeginTransComplete 事件在 BeginTrans 调用被完成后触发。 
  <LI>CommitTransComplete 事件在 CommitTrans 调用被完成后触发。 
  <LI>RollbackTransComplete 事件在 RollbackTrans 调用被完成后触发。 </LI></UL>
<H3>语法</H3><PRE>BeginTransComplete translevel,objerror,status,objconn
...
<TABLE class=dataintable>
  ...
  <TR>
    <TD>translevel</TD>
    <TD>Long 值，包含引发此事件的 BeginTrans 的新事务级别。</TD></TR>
  <TR>
    <TD>objerror</TD>
    <TD>包含所发生错误的 Error 对象。 
      <P>注释：<A href="#eventstatusenum">EventStatusEnum</A> 值必需设置为 
      adStatusErrorsOccurred ，以创建 Error 对象。</P></TD></TR>
  <TR>
    <TD>status</TD>
    <TD>规定事件执行的状态。取 <A href="#eventstatusenum">EventStatusEnum</A> 状态值。</TD></TR>
  <TR>
    <TD>objconn</TD>
    <TD>引发此事件的 Connection 对象。</TD></TR></TBODY></TABLE></DIV>
<DIV>
