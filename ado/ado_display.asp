...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO 显示</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="ADO 显示" href="ado_display.asp">ADO 显示</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ADO Recordset（记录集）" href="ado_recordset.asp">ADO 
  记录集</A> </LI>
  <LI class=next><A title="ADO 查询" href="ado_query.asp">ADO 查询</A> 
</LI></UL></DIV>
<DIV>
<P><STRONG>显示来自记录集中的数据的最常用的方法，就是把数据显示在 HTML 表格中。</STRONG></P></DIV>
<DIV class=example>
<H2>实例</H2>
<DL>
  <DT><A href="../tiy/s.asp@f=demo_adoe_display_1">显示记录</A> 
  <DD>如何首先创建一个数据库连接，然后创建一个记录集，然后把其中的数据显示在HTML中。 
  <DT><A href="../tiy/s.asp@f=demo_adoe_display_2">在 HTML 表格中显示记录</A> 
  <DD>如何把数据表中的数据显示在HTML表格中。 
  <DT><A href="../tiy/s.asp@f=demo_adoe_display_3">向 HTML 表格添加标题</A> 
  <DD>如何向HTML表格添加标题，以使其可读性更强。 
  <DT><A href="../tiy/s.asp@f=demo_adoe_display_4">向 HTML 表格添加颜色</A> 
  <DD>如何向HTML表格添加颜色，以使其更加美观。 </DD></DL></DIV>
<DIV>
<H2>显示字段名称和字段值</H2>
<P>我们有一个名为 "Northwind" 的数据库，并且我们希望显示出 "Customers" 表中的数据（记得以 .asp 
为扩展名来保存这个文件）：</P><PRE>&lt;html&gt;
&lt;body&gt;
...
<P>我们也可以通过下面的代码把表 "Customers" 中的数据显示在一个 HTML 表格中：</P><PRE>&lt;html&gt;
&lt;body&gt;
...
<DIV>
<H2>向 HTML 表格添加标题</H2>
<P>我们希望为这个 HTML 表格添加标题，这样它就更易读了：</P><PRE>&lt;html&gt;
&lt;body&gt;
