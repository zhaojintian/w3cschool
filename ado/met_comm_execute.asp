...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO Execute 方法</TITLE>
...
<P>如果 CommandText 属性指定以行返回的查询，执行产生的任何结果都将存储在新的 Recordset 
对象中。如果此命令不是以行返回的查询，提供者将返回关闭的 Recordset 对象。</P>
<H3>语法：对于以行返回的 Command：</H3><PRE>Set rs=objcommand.Execute(ra,parameters,options)</PRE>
<H3>语法：对于不是以行返回的 Command：</H3><PRE>objcommand.Execute ra,parameters,options</PRE>
<TABLE class=dataintable>
  ...
  <TR>
    <TD>ra</TD>
    <TD>可选。返回受查询影响的记录的数目。对于以行返回的查询，请使用 Recordset 对象的 RecordCount 
      属性来计算该对象中的记录数量。</TD></TR>
  <TR>
    <TD>parameters</TD>
    <TD>可选。用 SQL 语句传递的参数值。用于更改、更新或向 Parameters 集合插入新的参数值。</TD></TR>
  <TR>
    <TD>options</TD>
    <TD>可选。指示提供者应如何计算 Command 对象的 CommandText 属性。可以是一个或者多个 <A 
      href="app_commandtypeenum.asp">CommandTypeEnum</A> 或 <A 
      href="app_executeoptionenum.asp">ExecuteOptionEnum</A> 值。默认是 
      adCmdUnspecified。</TD></TR></TBODY></TABLE></DIV>
<DIV>
<H2>实例</H2><PRE>&lt;%
