...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO OpenSchema 方法</TITLE>
...
<DIV>
<H2>定义和用法</H2>
<P>OpenSchema 方法可返回 Recordset 对象，该对象包含有关数据源的模式信息。举例，schema 
信息可包括表的名称，表中的列名，每列的数据类型。Recordset 将以只读、静态游标模式打开。</P>
<H3>语法</H3><PRE>Set rs=objconn.OpenSchema(querytype,criteria,schemaid)</PRE>
<TABLE class=dataintable>
  ...
  <TR>
    ...
    <TD>
      ...
      <P>注释：OLEDB 规范只要求三个 SchemaEnum 值得到支持：adSchemaTables, adSchemaColumns 以及 
      adSchemaProviderTypes。</P></TD></TR>
  <TR>
    <TD>criteria</TD>
    <TD>可选。每个 QueryType 选项的查询约束的数组，如 SchemaEnum 中列出的内容。</TD></TR>
  <TR>
    <TD>schemaid</TD>
    <TD>OLE DB 规范未定义的提供者模式查询的 GUID。如果 QueryType 被设置为 
      adSchemaProviderSpecific，则需要此参数。否则，将不使用它。</TD></TR></TBODY></TABLE></DIV>
<DIV class=backtoreference>
<P><A title="ADO Connection 对象" href="ado_ref_connection.asp">返回 ADO Connection 
对象参考手册</A></P></DIV></DIV>
<DIV id=sidebar>
