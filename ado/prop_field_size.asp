...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO DefinedSize 和 ActualSize 属性</TITLE>
...
<P>ActualSize 属性是一个只读属性。它会返回一个 long 值，该值指示某个字段的值的实际长度。如果 ADO 无法确定该 Field 
对象的值的长度，则返回 adUnknown。</P>
<P>使用 DefinedSize 属性可确定 Field 对象的数据容量，而 ActualSize 则表明其实际长度。</P>
<H3>语法</H3><PRE>objrs.Fields(number).DefinedSize 
