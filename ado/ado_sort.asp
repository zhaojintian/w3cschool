...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO 排序</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="ADO 排序" href="ado_sort.asp">ADO 排序</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ADO 查询" href="ado_query.asp">ADO 查询</A> </LI>
  <LI class=next><A title="ADO 添加记录" href="ado_add.asp">ADO 添加</A> 
</LI></UL></DIV>
<DIV>
<P><STRONG>我们可以使用SQL来规定如何对记录集中的数据进行排序。</STRONG></P></DIV>
<DIV class=example>
<H2>实例</H2>
<DL>
  <DT><A href="../tiy/s.asp@f=demo_adoe_sort_1">根据指定的字段名处对记录进行升序排序</A> 
  <DD>如何根据指定字段名对数据进行排序 
  <DT><A href="../tiy/s.asp@f=demo_adoe_sort_2">根据指定的字段名处对记录进行降序排序</A> 
  <DD>如何根据指定字段名对数据进行排序 
  <DT><A href="../tiy/s.asp@f=demo_adoe_sort_3">让用户来选择根据哪列进行排序</A> 
  <DD>让用户来选择根据哪列进行排序 </DD></DL></DIV>
<DIV>
<H2>对数据进行排序</H2>
<P>我们希望显示 "Customers" 
表中的"Companyname"和"Contactname"字段，并根据"Companyname"进行排序（请记得用.asp为后缀保存）：</P><PRE>&lt;html&gt;
&lt;body&gt;
