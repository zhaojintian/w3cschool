...
<TABLE class=dataintable>
  ...
  <TR>
    ...
    <TD>0</TD>
    <TD>默认值。使用仅向前游标。除了在记录中只能向前滚动外，与静态游标相同。当只需要在 Recordset 
  中进行一个传递时，用它可提高性能。</TD></TR>
  <TR>
    <TD>adOpenKeyset</TD>
    <TD>1</TD>
    <TD>使用键集游标。尽管从您的 Recordset 
      不能访问其他用户删除的记录，但除了无法查看其他用户添加的记录外，它和动态游标相似。其他用户所作的数据更改依然可见。</TD></TR>
  <TR>
    <TD>adOpenDynamic</TD>
    <TD>2</TD>
    <TD>使用动态游标。其他用户所作的添加、更改或删除均可见，而且允许 Recordset 
  中的所有移动类型（如果提供者不支持书签，则书签除外）。</TD></TR>
  <TR>
    <TD>adOpenStatic</TD>
    <TD>3</TD>
    <TD>使用静态游标。一组记录的静态副本，可用于查找数据或生成报告。其他用户所作的添加、更改或删除均不可见。</TD></TR></TBODY></TABLE></DIV>
<DIV class=backtoreference>
