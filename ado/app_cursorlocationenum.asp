...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>CursorLocationEnum</TITLE>
...
<TABLE class=dataintable>
  ...
  <TR>
    <TD>adUseNone</TD>
    <TD>1</TD>
    <TD>不使用游标服务。（此常量已作废并且只是为了向后兼容才出现。）</TD></TR>
  <TR>
    <TD>adUseServer</TD>
    <TD>2</TD>
    <TD>默认值。使用数据提供者或驱动程序提供的游标。这些游标有时很灵活，可以额外感知其他人对数据源所做的更改。但是，Microsoft Cursor 
      Service for OLE DB 的某些功能（例如断开 Recordset 
对象）不能用服务器端游标模拟，这些功能在此设置中将不可用。</TD></TR>
  <TR>
    <TD>adUseClient</TD>
    <TD>3</TD>
    <TD>使用本地游标库提供的客户端的游标。本地游标服务通常允许执行驱动程序提供的游标所不允许的许多功能，因此使用此设置可以充分利用即将启用的功能。为进行向后兼容，亦支持同义字 
      adUseClientBatch。</TD></TR></TBODY></TABLE></DIV>
<DIV class=backtoreference>
