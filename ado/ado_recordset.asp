...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO Recordset（记录集）</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="ADO Recordset（记录集）" 
  href="ado_recordset.asp">ADO 记录集</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ADO 数据库连接" href="ado_connect.asp">ADO 连接</A> </LI>
  <LI class=next><A title="ADO 显示" href="ado_display.asp">ADO 显示</A> 
</LI></UL></DIV>
<DIV>
<P><STRONG>如需读取数据库的数据，那么其中的数据必须首先被载入一个记录集中。</STRONG></P></DIV>
<DIV>
<H2>创建一个 ADO 表记录集（ADO Table Recordset）</H2>
<P>在 ADO 数据库连接创建之后，如上一章所述，接下来就可以建立一个 ADO 记录集了。</P>
<P>假设我们有一个名为 "Northwind" 的数据库，我们可以通过下面的代码访问数据库中的 "Customers" 表：</P><PRE>&lt;%
...
<P>我们也可使用 SQL 访问 "Customers" 表中的数据：</P><PRE>&lt;%
set conn=Server.CreateObject("ADODB.Connection")
conn.Provider="Microsoft.Jet.OLEDB.4.0"
conn.Open "c:/webdata/northwind.mdb"
...
<P>假设我们用一个名为 "Northwind" 的数据库，我们可以通过下面的代码访问数据库中 "Customers" 表：</P><PRE>&lt;%
set conn=Server.CreateObject("ADODB.Connection")
conn.Provider="Microsoft.Jet.OLEDB.4.0"
conn.Open "c:/webdata/northwind.mdb"
...
<P>查看 ADO Recordset 对象的所有方法和属性。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="ADO 数据库连接" href="ado_connect.asp">ADO 连接</A> </LI>
  <LI class=next><A title="ADO 显示" href="ado_display.asp">ADO 显示</A> 
