...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO NativeError 属性</TITLE>
...
<P><A title="ADO Error 对象" href="ado_ref_error.asp">返回 ADO Error 
对象参考手册</A></P></DIV>
<DIV>
<H2>定义和用法</H2>
<P>NativeError 属性可返回一个 long 值，该值是由提供者或数据源返回 ADO 的错误代码。</P>
<P>使用 NativeError 属性对某个 Error 对象检索数据库特有的错误信息。例如，当配合使用 Microsoft ODBC Provider 
for OLE DB 和 Microsoft SQL Server 数据库时，从 SQL Server 产生的原生错误代码将通过 ODBC 和 ODBC 
Provider 传递到 ADO NativeError 属性。</P>
<H3>语法</H3><PRE>lngErrorNative=objErr.NativeError</PRE></DIV>
<DIV>
<H2>实例</H2><PRE>&lt;%
