...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ADO WillConnect, ConnectComplete 以及 Disconnect 事件</TITLE>
...
<BODY class=serverscripting>WillConnect, ConnectComplete 以及 Disconnect 事件 
<DIV id=wrapper>
...
<UL>
  <LI>WillConnect 事件在连接开始之前被触发。 
  <LI>ConnectComplete 事件在连接开始之后被触发。 
  <LI>Disconnect 事件在连接结束后被触发。 </LI></UL>
<H3>语法</H3><PRE>WillConnect ConnectionString,userid,psword,options,status,objcon
...
<TABLE class=dataintable>
  ...
  <TR>
    ...
    <TD>一个包含用于连接的信息的字符串。</TD></TR>
  <TR>
    <TD>userid</TD>
    <TD>一个包含用于连接的用户名的字符串。</TD></TR>
  <TR>
    <TD>psword</TD>
    <TD>一个包含用于连接的密码的字符串。</TD></TR>
  <TR>
    <TD>options</TD>
    <TD>Long 值，指示提供者计算 ConnectionString 的方式。唯一的选项是 adAsyncOpen。</TD></TR>
  <TR>
    <TD>objerror</TD>
    <TD>
      <P>包含所发生错误的 Error 对象。</P>
      <P>注释：<A href="#eventstatusenum">EventStatusEnum</A> 值必需设置为 
      adStatusErrorsOccurred ，以创建 Error 对象。</P></TD></TR>
  <TR>
    <TD>status</TD>
    <TD>
      <P>一个 <A href="#eventstatusenum">EventStatusEnum</A> 值。默认是 adStatusOK。</P>
      <P>当调用 ConnectComplete 时，如果 WillConnect 事件请求取消挂起的连接，此参数将设置为 
      adStatusCancel。</P></TD></TR>
  <TR>
    <TD>objconn</TD>
    <TD>应用此事件的 Connection 对象。</TD></TR></TBODY></TABLE></DIV>
<DIV>
