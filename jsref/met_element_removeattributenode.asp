...
<title>HTML DOM removeAttributeNode() 方法</title>
...
<h1>HTML DOM removeAttributeNode() 方法</h1>
...
<p class="tiy"><a  href="../tiy/t.asp@f=jsref_element_removeattributenode">亲自试一试</a></p>
</div>
...
<p>removeAttributeNode() 方法删除指定的属性，并以 Attr Node 对象返回被删除的属性。</p>

<p>此方法与 removeAttribute() 方法的差异是，removeAttribute() 方法返回具有指定名称的属性，而此方法删除指定的 Attr 对象。结果是相同的。同时，removeAttribute() 方法没有返回值，而此方法返回被删除的属性，以 Attr 对象的形式。</p>

<p>参阅 removeAttribute() 方法。</p>
</div>
...
<p>所有主流浏览器均支持 normalize() 方法，除了 Internet Explorer。</p>
</div>
...
<pre>element.removeAttributeNode(<i>attributenode</i>)</pre>
