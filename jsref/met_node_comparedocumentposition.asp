...
<title>HTML DOM compareDocumentPosition() 方法</title>
...
<h1>HTML DOM compareDocumentPosition() 方法</h1>
...
<p class="tiy"><a  href="../tiy/t.asp@f=jsref_node_compdocpos">亲自试一试</a></p>
</div>
...
<p>compareDocumentPosition() 方法比较两个节点，并返回描述它们在文档中位置的整数。</p>

<p>请看上面的例子。返回值可能是：</p>

<p>1：没有关系，两个节点不属于同一个文档。</p>

<p>2：第一节点（P1）位于第二个节点后（P2）。</p>

<p>4：第一节点（P1）定位在第二节点（P2）前。</p>

<p>8：第一节点（P1）位于第二节点内（P2）。</p>

<p>16：第二节点（P2）位于第一节点内（P1）。</p>

<p>32：没有关系，或是两个节点是同一元素的两个属性。</p>

<p class="note"><span>注释：</span>返回值可以是值的组合。例如，返回 20 意味着在 p2 在 p1 内部（16），并且 p1 在 p2 之前（4）。</p>
</div>
...
<p>所有主流浏览器均支持 compareDocumentPosition() 方法。</p>

<p class="note"><span>注释：</span>Internet Explorer 8 以及更早的版本不支持此方法。</p>
</div>
...
<pre>node.compareDocumentPosition(<i>node</i>)</pre>
