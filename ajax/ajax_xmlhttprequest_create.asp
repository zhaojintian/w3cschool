...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>AJAX - 创建 XMLHttpRequest 对象</TITLE>
...
<H2>AJAX XHR</H2>
<UL>
  <LI class=currentLink><A title="AJAX - 创建 XMLHttpRequest 对象" 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="AJAX 实例" href="ajax_example.asp">AJAX 实例</A> </LI>
  <LI class=next><A title="AJAX - 向服务器发送请求" 
  href="ajax_xmlhttprequest_send.asp">XHR 请求</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>XMLHttpRequest 是 AJAX 的基础。</STRONG></P></DIV>
<DIV>
<H2>XMLHttpRequest 对象</H2>
<P>所有现代浏览器均支持 XMLHttpRequest 对象（IE5 和 IE6 使用 ActiveXObject）。</P>
<P>XMLHttpRequest 用于在后台与服务器交换数据。这意味着可以在不重新加载整个网页的情况下，对网页的某部分进行更新。</P></DIV>
<DIV>
<H2>创建 XMLHttpRequest 对象</H2>
<P>所有现代浏览器（IE7+、Firefox、Chrome、Safari 以及 Opera）均内建 XMLHttpRequest 对象。</P>
<H3>创建 XMLHttpRequest 对象的语法：</H3><PRE>variable=new XMLHttpRequest();</PRE>
<H3>老版本的 Internet Explorer （IE5 和 IE6）使用 ActiveX 对象：</H3><PRE>variable=new ActiveXObject("Microsoft.XMLHTTP");</PRE>
<P>为了应对所有的现代浏览器，包括 IE5 和 IE6，请检查浏览器是否支持 XMLHttpRequest 对象。如果支持，则创建 
XMLHttpRequest 对象。如果不支持，则创建 ActiveXObject ：</P><PRE>var xmlhttp;
...
<P>在下一章中，您将学习发送服务器请求的知识。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="AJAX 实例" href="ajax_example.asp">AJAX 实例</A> </LI>
  <LI class=next><A title="AJAX - 向服务器发送请求" 
  href="ajax_xmlhttprequest_send.asp">XHR 请求</A> </LI></UL></DIV></DIV>
<DIV id=sidebar>
<DIV id=searchui>
