...
<HTML lang=zh-cn><HEAD><TITLE>jQuery Mobile 事件</TITLE>
<META charset=gbk>
...
<UL>
  <LI class=currentLink><A title="jQuery Mobile 事件" 
  href="jquerymobile_events_intro.asp">jQuery Mobile 事件</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="jQuery Mobile 主题" 
  href="jquerymobile_themes.asp">jQuery Mobile 主题</A> </LI>
  <LI class=next><A title="jQuery Mobile 触控" 
  href="jquerymobile_events_touch.asp">jQuery Mobile 触控</A> </LI></UL></DIV>
<DIV>
<P>事件 = 网页能够响应的所有不同访客的动作。</P></DIV>
<DIV>
<H2>jQuery Mobile 事件</H2>
<P>您能够在 jQuery Mobile 中使用任何标准的 <A title="jQuery 参考手册 - 事件" 
href="../jquery/jquery_ref_events.asp">jQuery 事件</A>。</P>
<P>此外，jQuery Mobile 还提供若干种为移动浏览定制的事件：</P>
<UL>
  <LI>触摸事件 - 当用户触摸屏幕时触发（敲击和滑动） 
  <LI>滚动事件 - 当上下滚动时触发 
  <LI>方向事件 - 当设备垂直或水平旋转时触发 
  <LI>页面事件 - 当页面被显示、隐藏、创建、加载以及/或卸载时触发 </LI></UL>
<P>如需关于所有 jQuery Mobile 事件的完整信息，请访问我们的 <A title="jQuery Mobile 事件" 
href="jquerymobile_ref_events.asp">jQuery Mobile 事件参考手册</A>。</P></DIV>
<DIV>
<H2>初始化 jQuery Mobile 事件</H2>
<P>在 jQuery 中，您已经学到使用文档 ready 事件来阻止 jQuery 代码在文档结束加载 (is ready) 前运行：</P>
<H3>jQuery document ready 事件</H3><PRE>&lt;script&gt;
$(document).ready(function(){

   <SPAN class=code_comment>// 此处是 jQuery 事件...</SPAN>

});
&lt;/script&gt;
</PRE>
<P><A href="../tiy/t.asp@f=jquerymobile_events_document_ready">亲自试一试</A></P>
<P>然而，在 jQuery Mobile 中，我们使用 pageinit 事件，该事件在页面已初始化并完善样式设置后发生。</P>
<P>第二个参数指向 ("#pageone") 指向页面的 idpoints to the id of the page to specify the 
events for:</P>
<H3>jQuery Mobile pageinit 事件</H3><PRE>&lt;script&gt;
$(document).<SPAN class=marked>on</SPAN>(<CODE>"pageinit"</CODE>,<CODE>"#pageone"</CODE>,function(){
...
<P><A href="../tiy/t.asp@f=jquerymobile_events_pageinitialized">亲自试一试</A></P>
<P class=note><SPAN>注释：</SPAN>jQuery on() 方法用于添加事件处理程序。</P>
<P>下几章将详解每个 jQuery Mobile 事件。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="jQuery Mobile 主题" 
  href="jquerymobile_themes.asp">jQuery Mobile 主题</A> </LI>
  <LI class=next><A title="jQuery Mobile 触控" 
  href="jquerymobile_events_touch.asp">jQuery Mobile 触控</A> </LI></UL></DIV></DIV><!-- maincontent end -->
<DIV id=sidebar>
<DIV id=searchui>
