...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>XQuery FLWOR 表达式</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="XQuery FLWOR 表达式" 
  href="xquery_flwor.asp">XQuery FLWOR</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="XQuery 实例" href="xquery_example.asp">XQuery 实例</A> 
  </LI>
  <LI class=next><A title="XQuery FLWOR + HTML" 
  href="xquery_flwor_html.asp">XQuery HTML</A> </LI></UL></DIV>
<DIV>
<H2>XML 实例文档</H2>
...
<P>请看下面这个路径表达式：</P><PRE>doc("books.xml")/bookstore/book[price&gt;30]/title</PRE>
<P>上面这个表达式可选取 bookstore 元素下的 book 元素下所有的 title 元素，并且其中的 price 元素的值必须大于 30。</P>
<P>下面这个 FLWOR 表达式所选取的数据和上面的路径表达式是相同的：</P><PRE><CODE>for</CODE> $x <CODE>in</CODE> doc("books.xml")/bookstore/book
<CODE>where</CODE> $x/price&gt;30
<CODE>return</CODE> $x/title</PRE>
<P>结果是：</P><PRE>&lt;title lang="en"&gt;XQuery Kick Start&lt;/title&gt;
&lt;title lang="en"&gt;Learning XML&lt;/title&gt;</PRE>
<P>通过 FLWOR，您可以对结果进行排序：</P><PRE><CODE>for</CODE> $x <CODE>in</CODE> doc("books.xml")/bookstore/book
<CODE>where</CODE> $x/price&gt;30
...
<P>上面的 XQuery 表达式的结果：</P><PRE>&lt;title lang="en"&gt;Learning XML&lt;/title&gt;
&lt;title lang="en"&gt;XQuery Kick Start&lt;/title&gt;</PRE></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="XQuery 实例" href="xquery_example.asp">XQuery 实例</A> 
  </LI>
  <LI class=next><A title="XQuery FLWOR + HTML" 
  href="xquery_flwor_html.asp">XQuery HTML</A> </LI></UL></DIV></DIV>
<DIV id=sidebar>
<DIV id=searchui>
