...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP File 对象</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="ASP File 对象" href="asp_ref_file.asp">ASP 
  File</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ASP Drive 对象" href="asp_ref_drive.asp">ASP Drive</A> 
  </LI>
  <LI class=next><A title="ASP Folder 对象" href="asp_ref_folder.asp">ASP 
  Folder</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>File 对象用于返回关于指定文件的信息。</STRONG></P></DIV>
<DIV class=example>
<H2>实例</H2>
<DL>
  <DT><A href="../tiy/s.asp@f=demo_aspe_datecreated">文件何时被创建？</A> 
  <DD>本例演示如何首先创建 FileSystemObject 对象，然后使用 File 对象的 DateCreated 
  属性来取得指定文件被创建的日期和时间。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_datelastmodified">此文件何时被修改？</A> 
  <DD>本例演示如何使用 DateLastModified 属性来取得指定文件被修改的日期和时间。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_datelastaccessed">此文件何时被访问过？</A> 
  <DD>此例演示如何使用 DateLastAccessed 属性来取得指定文件最后被访问的日期和时间。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_attributes">返回指定文件的属性</A> 
  <DD>本例演示如何使用 Attributes 来返回指定文件的属性。 </DD></DL></DIV>
<DIV>
<H2>File 对象</H2>
<P>File 对象用于返回有关指定文件的信息。</P>
<P>如需操作 File 对象的相关属性和方法，我们需要通过 FileSystemObject 来创建 File 对象的实例。首先，创建一个 
FileSystemObject 对象，然后通过 FileSystemObject 对象的 GetFile 方法，或者通过 Folder 对象的 Files 
属性来例示此 File 对象。</P>
<P>下面的代码使用 FileSystemObject 对象的 GetFile 方法来例示这个 File 对象，并使用 DateCreated 
属性来返回指定文件被创建的日期：</P><PRE>&lt;%
Dim fs,f
...
<P>输出：</P><PRE>File created: 8/8/2008 10:01:19 AM</PRE></DIV>
<DIV>
<H2>File 对象的属性和方法</H2>
<H3>属性</H3>
...
<TABLE class=dataintable>
  ...
  <TR>
    ...
    <TD>把指定文件从一个位置拷贝到另一个位置。</TD></TR>
  <TR>
    <TD><A href="met_delete_file.asp">Delete</A></TD>
    <TD>删除指定文件。</TD></TR>
  <TR>
    <TD><A href="met_move_file.asp">Move</A></TD>
    <TD>把指定文件从一个位置移动到另一个位置。</TD></TR>
  <TR>
    <TD><A href="met_openastextstream.asp">OpenAsTextStream</A></TD>
    <TD>打开指定文件，并返回一个 TextStream 对象以便访问此文件。</TD></TR></TBODY></TABLE></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="ASP Drive 对象" href="asp_ref_drive.asp">ASP Drive</A> 
  </LI>
  <LI class=next><A title="ASP Folder 对象" href="asp_ref_folder.asp">ASP 
  Folder</A> </LI></UL></DIV></DIV>
<DIV id=sidebar>
<DIV id=searchui>
