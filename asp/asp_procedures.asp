...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP 子程序</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="ASP 子程序" href="asp_procedures.asp">ASP 程序</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ASP 变量" href="asp_variables.asp">ASP 变量</A> </LI>
  <LI class=next><A title="ASP 表单和用户输入" href="asp_inputforms.asp">ASP 表单</A> 
  </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>在 ASP 中，你可通过 VBScript 和其他方式调用子程序。</STRONG></P></DIV>
<DIV class=example>
<H2>实例：</H2>
<DL>
  <DT><A href="../tiy/s.asp@f=demo_aspe_vbproc">调用使用 VBScript 的子程序</A> 
  <DD>如何从 ASP 调用以 VBScript 编写的子程序。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_jsproc">调用使用 JavaScript 的子程序</A> 
  <DD>如何从 ASP 调用以 JavaScript 编写的子程序。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_vbproc2">调用使用 VBScript 和 JavaScript 
  的子程序</A> 
  <DD>如何在一个 ASP 文件中调用以 VBScript 和 JavaScript 编写的子程序。 </DD></DL></DIV>
<DIV>
<H2>子程序</H2>
<P>ASP 源代码可包含子程序和函数：</P><PRE>&lt;html&gt;
&lt;head&gt;
<CODE>&lt;%
...
&lt;/html&gt;</PRE>
<P>将 &lt;%@ language="<I>language</I>" %&gt; 这一行写到 &lt;html&gt; 
标签的上面，就可以使用另外一种脚本语言来编写子程序或者函数：</P><PRE><CODE>&lt;%@ language="javascript" %&gt;</CODE>
&lt;html&gt;
&lt;head&gt;
&lt;%
...
<P>当从一个用 VBScript 编写的 ASP 文件中调用 VBScript 或者 JavaScript 子程序时，可以使用关键词 
"call"，后面跟着子程序名称。假如子程序需要参数，当使用关键词 "call" 时必须使用括号包围参数。假如省略 
"call"，参数则不必由括号包围。假如子程序没有参数，那么括号则是可选项。</P>
<P>当从一个用 JavaScript 编写的 ASP 文件中调用 VBScript 或者 JavaScript 
子程序时，必须在子程序名后使用括号。</P></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="ASP 变量" href="asp_variables.asp">ASP 变量</A> </LI>
  <LI class=next><A title="ASP 表单和用户输入" href="asp_inputforms.asp">ASP 表单</A> 
