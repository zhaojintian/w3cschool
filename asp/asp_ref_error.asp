...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP ASPError 对象</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="ASP ASPError 对象" href="asp_ref_error.asp">ASP 
  Error</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ASP Server 对象" href="asp_ref_server.asp">ASP 
  Server</A> </LI>
  <LI class=next><A title="ASP FileSystemObject 对象" 
  href="asp_ref_filesystem.asp">ASP FileSystem</A> </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>ASPError 对象用于显示在 ASP 文件的脚本中发生的任何错误的详细信息。</STRONG></P></DIV>
<DIV>
<H2>ASP ASPError 对象</H2>
<P>ASP 3.0 提供这个对象，且在 IIS5 及更高版本中可用。</P>
<P>ASPError 对象用于显示在 ASP 文件的脚本中发生的任何错误的详细信息。当 Server.GetLastError 被调用时，ASPError 
对象就会被创建，因此只能通过使用 Server.GetLastError 方法来访问错误信息。</P>
<P>ASPError 对象的属性描述如下（所有属性都是可读的）：</P>
<P class=note><SPAN>注释：</SPAN>下面的属性只能 <A title="ASP GetLastError() 方法 (ASP 3.0)" 
href="met_getlasterror.asp">Server.GetLastError()</A> 方法来访问。</P>
<H3>属性</H3>
