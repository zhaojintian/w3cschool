...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP - AJAX 与 ASP</TITLE>
...
<UL>
  ...
  <LI class=currentLink><A title="ASP - AJAX 与 ASP" href="asp_ajax_asp.asp">AJAX 
  ASP</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="AJAX 简介" href="asp_ajax_intro.asp">AJAX 简介</A> </LI>
  <LI class=next><A title="AJAX 数据库实例" href="asp_ajax_database.asp">AJAX 数据库</A> 
  </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>AJAX 用于创建动态性更强的应用程序。</STRONG></P></DIV>
<DIV>
<H2>AJAX ASP 实例</H2>
<P>下面的例子将演示当用户在输入框中键入字符时，网页如何与服务器进行通信：</P>
<H3>实例</H3>
<P>请在下面的输入框中键入字母（A - Z）：</P>
<FORM style="MARGIN-TOP: 15px" action="">姓名：<INPUT onkeyup=showHint(this.value) 
id=txt1> </FORM>
<P>建议：<SPAN id=txtHint></SPAN></P>
<P class=tiy><A href="../tiy/t.asp@f=ajax_suggest">亲自试一下源代码</A></P></DIV>
<DIV>
<H2>实例解释 - HTML 页面</H2>
<P>当用户在上面的输入框中键入字符时，会执行 "showHint()" 函数。该函数由 "onkeyup" 事件触发：</P><PRE>&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
&lt;script&gt;
function showHint(str)
...
<UL>
  ...
  <LI>创建在服务器响应就绪时执行的函数 
  <LI>向服务器上的文件发送请求 
  <LI>请注意添加到 URL 末端的参数（q）（包含输入框的内容） </LI></UL></DIV>
<DIV>
<H2>ASP 文件</H2>
<P>上面这段 JavaScript 调用的服务器页面是名为 "gethint.asp" 的 ASP 文件。</P>
<P>"gethint.asp" 中的源代码会检查姓名数组，然后向浏览器返回对应的姓名：</P><PRE>&lt;%
response.expires=-1
dim a(30)
...
<UL>
  <LI>查找匹配来自 JavaScript 的字符的姓名 
  <LI>如果未找到匹配，则将响应字符串设置为 "no suggestion" 
  <LI>如果找到一个或多个匹配姓名，则用所有姓名设置响应字符串 
  <LI>把响应发送到占位符 "txtHint" </LI></UL></DIV>
<DIV id=bpn>
<UL class=prenext>
  <LI class=pre><A title="AJAX 简介" href="asp_ajax_intro.asp">AJAX 简介</A> </LI>
  <LI class=next><A title="AJAX 数据库实例" href="asp_ajax_database.asp">AJAX 数据库</A> 
