...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP Form 集合</TITLE>
...
<P class=note><SPAN>注释：</SPAN>如果您需要 post 大量的数据（超过 100kb），就不能使用 Request.Form 
。</P></DIV>
<DIV>
<H2>语法</H2><PRE>Request.Form(element)[(index)|.Count]</PRE>
<TABLE class=dataintable>
  ...
  <TR>
    <TD>element </TD>
    <TD>必需。表单元素的名称，此集合从中取回值。</TD></TR>
  <TR>
    <TD>index </TD>
    <TD>可选。规定一个参数的多个值中的一个。从 1 到 
Request.Form(parameter).Count。</TD></TR></TBODY></TABLE></DIV>
<DIV>
<H2>实例</H2>
<P><B>例子 1</B></P>
<P>您可以遍历表单请求中的所有值。假设用户通过指定两个值来填写了表单 - 蓝色和绿色 - 您能够这样取回这些值：</P><PRE>&lt;%
...
<P><B>例子 2</B></P>
<P>请认真看一下这个表单：</P><PRE>&lt;form action="submit.asp" method="post"&gt;
...
<P>假设，发送了下面的请求：</P><PRE>firstname=John&amp;lastname=Dove&amp;color=Red</PRE>
<P>现在，我们可以通过一段脚本来使用表单中的信息了：</P><PRE>Hi, &lt;%=Request.Form("firstname")%&gt;.&nbsp;
Your favorite color is &lt;%=Request.Form("color")%&gt;.
</PRE>
<P>输出：</P><PRE>Hi, John. Your favorite color is Red.</PRE>
<P>如果您不规定要显示的元素：</P><PRE>Form data is: &lt;%=Request.Form%&gt;&nbsp;</PRE>
<P>那么输出会成为这样：</P><PRE>Form data is: firstname=John&amp;lastname=Dove&amp;color=Red</PRE></DIV>
<DIV>
<P><A title="ASP Request 对象" href="asp_ref_request.asp">Request 
对象参考手册</A></P></DIV></DIV>
<DIV id=sidebar>
