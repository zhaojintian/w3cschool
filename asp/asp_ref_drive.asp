...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP Drive 对象</TITLE>
...
<UL>
  ...
  TextStream</A> 
  <LI class=currentLink><A title="ASP Drive 对象" href="asp_ref_drive.asp">ASP 
  Drive</A> 
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ASP TextStream 对象" href="asp_ref_textstream.asp">ASP 
  TextStream</A> </LI>
  <LI class=next><A title="ASP File 对象" href="asp_ref_file.asp">ASP File</A> 
  </LI></UL></DIV>
<DIV id=intro>
<P><STRONG>Drive 对象用于返回关于本地磁盘驱动器或者网络共享驱动器的信息。</STRONG></P></DIV>
<DIV class=example>
<H2>实例</H2>
<DL>
  <DT><A href="../tiy/s.asp@f=demo_aspe_availablespace">取得指定驱动器的可用空间数</A> 
  <DD>本例演示如何首先创建一个 FileSystemObject 对象，然后使用 AvailableSpace 属性来获得指定驱动器的可用空间。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_freespace">取得指定驱动器的剩余空间容量</A> 
  <DD>本例演示如何使用 FreeSpace 空间属性来取得指定驱动器的剩余空间。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_totalsize">取得指定驱动器的总容量</A> 
  <DD>本例演示如何使用 TotalSize 属性来获得指定驱动器的总容量。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_driveletter">取得指定驱动器的驱动器字母</A> 
  <DD>本例演示如何使用 DriveLetter 属性来获得指定驱动器的驱动器字母。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_drivetype">取得指定驱动器的驱动器类型</A> 
  <DD>本例演示如何使用 DriveType 属性来获得指定驱动器的驱动器类型。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_filesystem">取得指定驱动器的文件系统信息</A> 
  <DD>本例演示如何使用 FileSystem 来取得指定驱动器的文件系统类型。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_isready">驱动器是否已就绪？</A> 
  <DD>本例演示如何使用 IsReady 属性来检查指定的驱动器是否已就绪。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_path">取得指定驱动器的路径</A> 
  <DD>本例演示如何使用 Path 属性来取得指定驱动器的路径。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_rootfolder">取得指定驱动器的根文件夹</A> 
  <DD>本例演示如何使用 RootFolder 属性来取得指定驱动器的根文件夹。 
  <DT><A href="../tiy/s.asp@f=demo_aspe_serialnumber">取得指定驱动器的序列号</A> 
  <DD>本例演示如何使用 Serialnumber 属性来取得指定驱动器的序列号。 </DD></DL></DIV>
<DIV>
<H2>Drive 对象</H2>
<P>Drive 对象用于返回关于本地磁盘驱动器或者网络共享驱动器的信息。Drive 对象可以返回有关驱动器的文件系统、剩余容量、序列号、卷标名等信息。</P>
<P class=note><SPAN>注释：</SPAN>无法通过 Drive 对象返回有关驱动器内容的信息。要达到这个目的，请使用 Folder 
对象。</P>
<P>如需操作 Drive 对象的相关属性，我们需要创建通过 FileSystemObject 对象来创建 Drive 对象的实例。首先，创建一个 
FileSystemObject 对象，然后通过 FileSystemObject 对象的 GetDrive 方法或者 Drives 属性来例示 Drive 
对象。</P>
<P>下面的例子使用 FileSystemObject 对象的 GetDrive 方法来例示 Drive 对象，并使用 TotalSize 属性来返回指定驱动器 
(c:) 的容量总数（字节）：</P><PRE>&lt;%
Dim fs,d
...
<P>输出：</P>
<P>Drive c: Total size in bytes: 5893563398</P></DIV>
<DIV>
<H2>Drive 对象的属性</H2>
