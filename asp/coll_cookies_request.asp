...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP Cookies 集合</TITLE>
...
<H3>语法：</H3><PRE>Response.Cookies(name)[(key)|.attribute]=value

variablename=Request.Cookies(name)[(key)|.attribute]</PRE></DIV>
<DIV>
<TABLE class=dataintable>
  ...
  <TR>
    <TD>name </TD>
    <TD>必需。cookie 的名称。</TD></TR>
  <TR>
    <TD>value </TD>
    <TD>必需（对于 Response.Cookies 命令）。cookie 的值。</TD></TR>
  <TR>
    <TD>attribute </TD>
    <TD>
      <P>可选。规定有关 cookie 的信息。可以是下面的参数之一。</P>
      <UL class=listintable>
        <LI>Domain - 只写；cookie 仅送往到达该域的请求。 
        <LI>Expires - 只写；cookie 的失效日期。如果没有规定日期，cookie 会在 session 结束时失效。 
        <LI>HasKeys - 只读；规定 cookie 是否拥有 key（这是唯一一个可与 Request.Cookies 命令使用的属性） 
        <LI>Path - 只写；如果设置，cookie 仅送往到达此路径的请求。如果没有设置，则使用应用程序的路径。 
        <LI>Secure - 只写；指示 cookie 是否安全。 </LI></UL></TD></TR>
  <TR>
    <TD>key </TD>
    <TD>可选。规定在何处赋值的 key。</TD></TR></TBODY></TABLE></DIV>
<DIV>
<H2>实例</H2>
<P>"Response.Cookies" 命令用于创建 cookie 或者设置 cookie 的值：</P><PRE>&lt;%
Response.Cookies("firstname")="Alex"
%&gt;
</PRE>
<P>在上面的代码中，我们创建了一个名为 "firstname" 的 cookie，并为它赋值 alex 。</P>
<P>也可以为 cookie 设置属性，比如设置 cookie 的失效时间：</P><PRE>&lt;%
Response.Cookies("firstname")="Alex"&nbsp;
...
<P>在下面的例子中，我们取回了 cookie "firstname" 的值，并把它显示到页面上：</P><PRE>&lt;%
...
<P>在下面的例子中，我们要创建一个名为 "user" 的 cookie 集合。"user" cookie 拥有包含有关用户信息的 key 。</P><PRE>&lt;%
...
<P>下面的代码可读出所有服务器已向用户发送的 cookie 。请注意，我们使用了 HasKeys 属性来判断 cookie 是否拥有 key ：</P><PRE>&lt;html&gt;
&lt;body&gt;
