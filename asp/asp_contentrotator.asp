...
<HTML 
xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>ASP Content Rotator</TITLE>
...
<UL>
  ...
  ContentLinking</A> 
  <LI class=currentLink><A title="ASP Content Rotator (ASP 3.0)" 
  href="asp_contentrotator.asp">ASP ContentRotator</A> </LI></UL>
<H2>AJAX 与 ASP</H2>
<UL>
...
<DIV id=tpn>
<UL class=prenext>
  <LI class=pre><A title="ASP Content Linking 组件" 
  href="asp_contentlinking.asp">ASP ContentLinking</A> </LI>
  <LI class=next><A title="AJAX 简介" href="asp_ajax_intro.asp">AJAX 简介</A> 
</LI></UL></DIV>
<DIV class=example>
<H2>实例</H2>
<DL>
  <DT><A href="../tiy/s.asp@f=demo_aspe_contentrotator">Content Rotator 组件</A> 
  <DD>每当用户访问或者刷新页面时，该组件就会显示不同的 HTML 内容字符串。 </DD></DL></DIV>
<DIV>
<H2>ASP Content Rotator 组件</H2>
<P>ASP Content Rotator 组件会创建一个 ContentRotator 对象，每当用户访问或者刷新某个页面时，该对象就会显示一段不同的 
HTML 内容字符串。一个名为内容目录文件（Content Schedule File）的文本文件包含着有关内容字符串的信息。</P>
...
<P class=important><SPAN>注意：</SPAN>在每个内容字符串起始位置的#号码。这个号码是一个可选的参数，用来 HTML 
内容字符串的相对权重。在本例中，Content Rotator 
有十分之一的几率显示第一个内容字符串，有十分之二的几率显示第二个内容字符串，有十分之三的几率显示第三个字符串，而第四个字符串为十分之四的几率。</P>
<P>然后，创建一个 ASP 文件，并插入下面的代码：</P><PRE>&lt;html&gt;
&lt;body&gt;
